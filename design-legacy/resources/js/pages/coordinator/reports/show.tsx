...
const selectStyle = { padding: '8px 10px', border: '1px solid #DDD', borderRadius: 8, width: '100%' } as const;
...
export default function CoordinatorReportShow({ report, communities, causeOptions, actionOptions, exports }: Props) {
    ...
    return (
        <AdminLayout>
            ...
            <div className="page-title">{report.company_name} — {report.period_key}</div>
            <div className="page-sub">
                لقطة ثابتة لا يُعاد حسابها · وُلِّدت {report.generated_at ? new Date(report.generated_at).toLocaleDateString('ar-SA') : '—'}
            </div>
            <Link href="/coordinator/reports" style={{ fontSize: 12, color: '#E03050' }}>← كل التقارير</Link>

            <div className="stat-row" style={{ marginTop: 16 }}>
                <StatCard emoji="✅" label="معدل التفعيل" value={`${s.activation_rate.rate}%`} change={`${s.activation_rate.numerator} من ${s.activation_rate.denominator}`} color="#0CA678" />
                <StatCard emoji="📅" label="الفعاليات المكتملة" value={s.completed_events} change={`${s.created_events} أُنشئت`} color="#3B5BDB" />
                <StatCard emoji="😴" label="المجتمعات الخاملة" value={s.communities.dormant.length} change={`${s.communities.active.length} نشطاً`} color="#E03050" />
                <StatCard emoji="🚫" label="معدل الإلغاء" value={`${s.cancellation_rate.rate}%`} change={`${s.cancellation_rate.numerator} من ${s.cancellation_rate.denominator}`} color="#D4820A" />
            </div>

            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16 }}>
                {exports.map((item) => (
                    <span key={item.key} style={{ display: 'inline-flex', border: '1px solid #232A3E', borderRadius: 8, overflow: 'hidden', fontSize: 12 }}>
                        <span style={{ padding: '8px 10px', opacity: 0.75 }}>{item.title}</span>
                        <a href={`/coordinator/reports/${report.id}/export/${item.key}?format=xlsx`} style={{ padding: '8px 10px', borderRight: '1px solid #232A3E', color: '#E03050' }}>Excel</a>
                        <a href={`/coordinator/reports/${report.id}/export/${item.key}?format=pdf`} target="_blank" rel="noreferrer" style={{ padding: '8px 10px', borderRight: '1px solid #232A3E', color: '#E03050' }}>PDF</a>
                    </span>
                ))}
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
                <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
                    <div style={{ padding: '14px 20px', borderBottom: '1px solid #232A3E', fontSize: 14, fontWeight: 700 }}>المجتمعات الخاملة</div>
                    <table className="portal-table">
                        <tbody>
                ...
                <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
                    <div style={{ padding: '14px 20px', borderBottom: '1px solid #232A3E', fontSize: 14, fontWeight: 700 }}>أسباب الإلغاء</div>
                    <table className="portal-table">
                        <tbody>
