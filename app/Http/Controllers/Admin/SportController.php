...
class SportController extends Controller
{
    ...
    {
        $query = Sport::withTrashed()->withCount(['communities', 'courts', 'events']);
        ...
        $totalSports = Sport::withTrashed()->count();
    ...
    public function store(Request $request): RedirectResponse
    {
        $data = $request->validate([
            ...
            'name.required' => 'اسم الرياضة مطلوب.',
            'name.max' => 'اسم الرياضة يجب ألا يتجاوز 100 حرف.',
            'icon.image' => 'الأيقونة يجب أن تكون صورة.',
            'icon.max' => 'حجم الأيقونة يجب ألا يتجاوز 2 ميجابايت.',
        ]);

        if ($request->hasFile('icon')) {
            $storedPath = $request->file('icon')->store('sports', 'public');
    ...
    public function update(Request $request, int $id): RedirectResponse
    {
        ...
        if ($request->hasFile('icon')) {
            ...
            if ($sport->icon && str_starts_with($sport->icon, '/storage/')) {
                Storage::disk('public')->delete(str_replace('/storage/', '', $sport->icon));
            }
            $storedPath = $request->file('icon')->store('sports', 'public');
            $data['icon'] = '/storage/'.$storedPath;
            $data['color'] = $this->extractDominantColor($storedPath) ?? '#009E82';
