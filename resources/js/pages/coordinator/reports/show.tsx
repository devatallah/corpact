...
import { Download, FileText, Plus, Trash2, TrendingDown, TrendingUp } from 'lucide-react';
...
import { Badge, Button, Card, INPUT, Note, PageHeader, StatCard, TableShell, Tbody, Td, Th, Thead, Tr } from '@/components/portal/ui';
import AdminLayout from '@/layouts/admin-layout';
import { REPORT_STATUS } from '@/pages/coordinator/reports/index';
 ...
 */
type Kpi = { key: string; label: string; numerator: number; denominator: number; rate: number; formula: string };
...
export default function CoordinatorReportShow({
    ...
    report: {
        ...
        snapshot: {
            ...
            communities: { window_days: number; dormant: { id: number; name: string }[]; metric: Kpi };
    ...
    const form = useForm<{ recommendations: Draft[]; note: string }>({
        recommendations: report.recommendations.map((row) => ({
            cause: row.cause,
            action: row.action,
            community_id: row.community_id === null ? '' : String(row.community_id),
        })),
        note: report.note ?? '',
    });
    ...
    const update = (index: number, patch: Partial<Draft>) => {
        form.setData(
            'recommendations',
            form.data.recommendations.map((row, position) => (position === index ? { ...row, ...patch } : row)),
        );
    };

    const complete = form.data.recommendations.every((row) => row.cause && row.action);
    ...
    return (
        <AdminLayout>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <DeltaCard label="معدل التفعيل" delta={mom.activation_rate} suffix="٪" />
                <DeltaCard label="معدل الحضور" delta={mom.attendance_rate} suffix="٪" />
                <DeltaCard label="معدل الإلغاء" delta={mom.cancellation_rate} suffix="٪" goodWhenDown />
            </div>

            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <StatCard label="فعاليات مكتملة" value={snapshot.completed_events} />
                <StatCard label="فعاليات أُنشئت" value={snapshot.created_events} />
                <StatCard label="مشاركات موثّقة" value={snapshot.attendance_count} />
                <StatCard
                    label="مجتمعات خاملة"
                    value={snapshot.communities.dormant.length}
                    tone={snapshot.communities.dormant.length > 0 ? 'warning' : 'success'}
            ...
            {snapshot.communities.dormant.length > 0 && (
                <Card padding="p-4" className="space-y-2">
                    <h2 className="text-sm font-extrabold text-ink">المجتمعات الخاملة</h2>
                    ...
                    <p className="text-[11px] text-ink/55">
                        هذه هي المرشّحة لتوصية — اربط كل واحدة بسبب وإجراء أدناه.
                    </p>
                </Card>
            )}
            ...
            <Card padding="p-4" className="space-y-3">
                <h2 className="text-sm font-extrabold text-ink">أسباب الإلغاء</h2>
                <div className="flex flex-wrap gap-2">
                    {snapshot.cancellation_reasons.map((reason) => (
                        <span
                            key={reason.status}
                            className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-[11px] font-bold bg-ink/5 border-[0.5px] border-ink/12"
                        >
                            {reason.label}
                            <span className="font-mono font-black text-ink">{reason.count}</span>
                        </span>
                    ))}
                    {snapshot.cancellation_reasons.length === 0 && <span className="text-xs text-ink/55">لا إلغاءات.</span>}
                </div>
            </Card>
                ...
                >
                    <div className="space-y-3">
                        {form.data.recommendations.map((row, index) => (
                            <div key={index} className="rounded-2xl border-[0.5px] border-ink/12 bg-page p-3 space-y-2">
                                <div className="flex items-center justify-between gap-2">
                                    <span className="text-[11px] font-bold text-ink/60">توصية {index + 1}</span>
                                    <button
                                        type="button"
                                        onClick={() =>
                                            form.setData(
                                                'recommendations',
                                                form.data.recommendations.filter((_, position) => position !== index),
                                            )
                                        }
                                        aria-label="حذف التوصية"
                                        className="p-1 rounded-lg bg-danger/8 hover:bg-danger/15 text-danger transition-colors"
                                    >
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                                    <select
                                        ...
                                        onChange={(event) => update(index, { community_id: event.target.value })}
                                    >
                    ...
                    <Button
                        ...
                        onClick={() =>
                            form.setData('recommendations', [...form.data.recommendations, { cause: '', action: '', community_id: '' }])
                        }
                    >
                        توصية أخرى
                    </Button>
                    ...
                    <div>
                        <label htmlFor="report-note" className="block text-[11px] font-bold text-ink mb-1.5">
                        ...
                        />
                        {form.errors.note && <p className="text-[11px] text-danger mt-1">{form.errors.note}</p>}
                    </div>

                    <Note tone="warning" title="الحفظ يُرسل التقرير">
                        حفظ التوصيات يختم التقرير كـ«مُرسَل» ويسجّل وقت الإرسال. أكمل توصياتك قبل الحفظ.
                    </Note>
                </FormSection>

                <FormActions cancelHref="/coordinator/reports">
                    <Button type="submit" disabled={form.processing || !complete}>
                        حفظ وإرسال التقرير
                    </Button>
                </FormActions>
            </form>
            ...
            <Card padding="p-4" className="space-y-4">
                <div className="flex items-center justify-between gap-2 flex-wrap">
                    <h2 className="text-sm font-extrabold text-ink">لقطة القياس</h2>
                    <span className="font-mono text-[10px] text-ink/45">
                        وُلِّدت {report.generated_at ? new Date(report.generated_at).toLocaleString('ar-SA') : '—'}
                    </span>
                </div>
                ...
                <TableShell>
                    ...
                    <Tbody>
                        {[snapshot.activation_rate, snapshot.attendance_rate, snapshot.cancellation_rate].map((kpi) => (
                            <Tr key={kpi.key}>
                                <Td className="font-extrabold text-ink">{kpi.label}</Td>
                                <Td className="font-mono font-black text-ink whitespace-nowrap">
                                    {kpi.rate}٪
                                    <span className="block text-[10px] font-normal text-ink/50">
                                        {kpi.numerator} / {kpi.denominator}
                                    </span>
                                </Td>
                                <Td className="text-[11px] text-ink/60 max-w-md">{kpi.formula}</Td>
                            </Tr>
                        ))}
                    </Tbody>
                </TableShell>
                ...
                <Note title="اللقطة لا تُعدَّل">
                    قيم هذا التقرير قيست وقت توليده وحُفظت كما هي. تغيّر البيانات لاحقاً لا يعيد كتابة تقرير سُلّم للشركة.
                </Note>
            </Card>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {exports.map((row) => (
                            <a
                                ...
                                className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-full bg-ink text-lime text-xs font-bold hover:opacity-90 transition-opacity"
                            >
            ...
            <ConfirmModal
                ...
                details={
                    <>
                        <ConfirmRow label="الشركة" value={report.company_name} strong />
                        <ConfirmRow label="الدورة" value={report.period_key} />
                        <ConfirmRow label="عدد التوصيات" value={`${form.data.recommendations.length}`} strong />
                        <ConfirmRow label="ملاحظة" value={form.data.note.trim() ? 'مرفقة' : 'بلا ملاحظة'} />
                    </>
                }
                confirmLabel="نعم، احفظ وأرسل"
                onConfirm={() => {
                    form.post(`/coordinator/reports/${report.id}/recommendations`, { preserveScroll: true });
                    setSaving(false);
                }}
