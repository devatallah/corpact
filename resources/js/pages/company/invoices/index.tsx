...
export default function CompanyInvoices({ company, invoices, filters, summary, sort }: Props) {
    ...
    return (
        <CompanyLayout>
            ...
            <div className="page-title">الفواتير الشهرية</div>
            ...
            {summary.event_creation_blocked && (
                ...
                >
                    <div style={{ fontWeight: 700, color: '#E03050', marginBottom: 4 }}>إنشاء الفعاليات الجديدة موقوف للتأخر</div>
                    <div style={{ fontSize: 12, color: '#43506B', lineHeight: 1.9 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(200px,1fr))', gap: 12, margin: '16px 0' }}>
                ...
                <StatCard
                    ...
                    color={summary.overdue_count > 0 ? '#E03050' : undefined}
                />
            </div>
                    ...
                    <table className="portal-table">
                        ...
                        <tbody>
                            ...
                            {invoices.data.map((invoice) => (
                                <tr key={invoice.id}>
                                    <td dir="ltr" style={{ fontWeight: 700, color: '#1B2437' }}>{invoice.serial ?? `#${invoice.id}`}</td>
                                    <td style={{ fontSize: 12, color: '#43506B' }} dir="ltr">
                                        {invoice.period_start} → {invoice.period_end}
                                    </td>
                                    <td>{invoice.activated_employees_count}</td>
                                    <td style={{ fontWeight: 700 }} dir="ltr">{invoice.total_amount}</td>
                                    <td style={{ fontSize: 12, color: invoice.is_overdue ? '#E03050' : '#7A8BA8' }}>
