...
function ChangeBadge({ change, invert }: { change: number; invert?: boolean }) {
    ...
    const color = change === 0 ? '#888' : good ? '#059669' : '#DC2626';
    const sign = change > 0 ? '+' : '';
    ...
    return (
        <CompanyLayout>
            ...
            <div style={{ marginBottom: 20 }}>
                <div className="page-title">التقرير الشهري — {report.period_key}</div>
                <div className="page-sub">
                    {company.name} · لقطة ثابتة وُلِّدت في {report.generated_at ? new Date(report.generated_at).toLocaleDateString('ar-SA') : '—'}
                </div>
                <Link href="/company/reports" style={{ fontSize: 12, color: '#1A56DB' }}>← عودة إلى التقارير</Link>
            </div>

            <div className="stat-row">
                <StatCard emoji="✅" label="معدل التفعيل" value={`${s.activation_rate.rate}%`} change={`${s.activation_rate.numerator} من ${s.activation_rate.denominator}`} color="#059669" />
                <StatCard emoji="📅" label="الفعاليات المكتملة" value={s.completed_events} change={`${s.created_events} أُنشئت`} color="#3B5BDB" />
                <StatCard emoji="😴" label="المجتمعات الخاملة" value={s.communities.dormant.length} change={`${s.communities.active.length} نشطاً`} color="#DC2626" />
                <StatCard emoji="💸" label="التكلفة لكل مشاركة" value={`${s.cost_per_participation} ر.س`} change={`إنفاق ${s.company_spend}`} color="#B45309" />
            </div>

            <div className="card" style={{ padding: 0, overflow: 'hidden', marginBottom: 16 }}>
                <div style={{ padding: '14px 20px', borderBottom: '1px solid #F0EDE8', fontSize: 14, fontWeight: 700 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
                <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
                    <div style={{ padding: '14px 20px', borderBottom: '1px solid #F0EDE8', fontSize: 14, fontWeight: 700 }}>المجتمعات الخاملة</div>
                    <table className="portal-table">
                        <tbody>
                            {s.communities.dormant.length === 0 ? (
                                <tr><td style={{ ...tdStyle, textAlign: 'center', color: '#888', padding: 24 }}>لا مجتمع خاملاً — كل المجتمعات أقامت فعالية مكتملة.</td></tr>
                            ) : (
                ...
                <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
                    <div style={{ padding: '14px 20px', borderBottom: '1px solid #F0EDE8', fontSize: 14, fontWeight: 700 }}>أسباب الإلغاء</div>
                    <table className="portal-table">
                        <tbody>
                            {s.cancellation_reasons.every((row) => row.count === 0) ? (
                                <tr><td style={{ ...tdStyle, textAlign: 'center', color: '#888', padding: 24 }}>لا إلغاءات في هذه الدورة.</td></tr>
                            ) : (
            ...
            <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
                ...
                {report.note && (
                    <div style={{ padding: '14px 20px', borderTop: '1px solid #F0EDE8', fontSize: 13, background: '#FFF8E6' }}>
                        <strong style={{ fontSize: 12, color: '#888' }}>ملاحظة المنسّق:</strong>
