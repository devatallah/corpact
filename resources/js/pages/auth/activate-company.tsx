...
export default function ActivateCompany({
    ...
    if (activated) {
        return (
            <AuthLayout title="تم التفعيل بنجاح" subtitle={`حساب ${companyName} جاهز الآن.`}>
    ...
    return (
        <AuthLayout title="تفعيل الحساب" subtitle={`اختر كلمة مرور لحساب ${companyName}.`}>
            ...
            <form
                onSubmit={(event) => {
                    ...
                    form.post(`/company/activate/${token}`);
                }}
                className="space-y-5"
            >
