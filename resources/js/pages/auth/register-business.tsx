            ...
            <>
                ...
                <div style={{ width: '100%', maxWidth: 520 }} dir="rtl">
                    ...
                    <div style={{ background: '#fff', borderRadius: 24, padding: '36px 32px', boxShadow: '0 24px 80px rgba(0,0,0,.5)', textAlign: 'center' }}>
                        ...
                        <a href="/business/login" style={{ display: 'block', textAlign: 'center', fontSize: 13, color: '#C8410A', fontWeight: 700, textDecoration: 'none' }}>← العودة للصفحة الرئيسية</a>
                    </div>
                </div>
            </>
        );
        ...
        <>
            ...
            <div style={{ width: '100%', maxWidth: 520 }} dir="rtl">
                ...
                <div style={{ background: '#fff', borderRadius: 24, padding: '36px 32px', boxShadow: '0 24px 80px rgba(0,0,0,.5)' }}>
                    <div style={{ fontSize: 20, fontWeight: 900, color: '#1C1410', marginBottom: 4 }}>تسجيل منشأة جديدة</div>
                    ...
                    <div style={{ background: '#C8410A10', border: '1px solid #C8410A33', borderRadius: 12, padding: '14px 16px', marginBottom: 20, fontSize: 12, color: '#C8410A', lineHeight: 1.7 }}>
                        <strong style={{ display: 'block', fontSize: 13, marginBottom: 4 }}>🏟️ ماذا تستفيد كمنشأة؟</strong>
                    ...
                    <form onSubmit={submit}>
                        ...
                        <div style={{ fontSize: 13, fontWeight: 800, color: '#1C1410', margin: '20px 0 14px', paddingBottom: 8, borderBottom: '1px solid #EAE4DC' }}>بيانات المنشأة</div>

                        <div style={{ marginBottom: 16 }}>
                            <label style={{ display: 'block', fontSize: 12, color: '#4A5C78', fontWeight: 600, marginBottom: 6 }}>اسم المنشأة</label>
                            <input className="rc-inp" type="text" placeholder="منشأة الفئة الحديثة" value={data.name} onChange={e => setData('name', e.target.value)} />
                        </div>
                        ...
                        {/* Category checkboxes */}
                        <div style={{ marginBottom: 16 }}>
                            <label style={{ display: 'block', fontSize: 12, color: '#4A5C78', fontWeight: 600, marginBottom: 6 }}>الفئات المتاحة في المنشأة</label>
                            {categories.map(cat => (
                                <div key={cat.id} style={{ marginBottom: 10 }}>
                                    {cat.children && cat.children.length > 0 ? (
                                        <>
                                            <div style={{ fontSize: 12, fontWeight: 700, color: '#8A7868', marginBottom: 6 }}>{cat.name}</div>
                                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                                                {cat.children.map(sub => {
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            <input type="checkbox" checked={checked} readOnly style={{ width: 16, height: 16, accentColor: '#C8410A', cursor: 'pointer' }} />
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                            <div style={{ marginBottom: 16 }}>
                                <label style={{ display: 'block', fontSize: 12, color: '#4A5C78', fontWeight: 600, marginBottom: 6 }}>عدد المرافق الإجمالي</label>
                                <input className="rc-inp" type="number" dir="ltr" placeholder="4" min={1} value={data.venues_count} onChange={e => setData('venues_count', parseInt(e.target.value) || 1)} />
                            </div>
                            <div style={{ marginBottom: 16 }}>
                        ...
                        <div style={{ fontSize: 13, fontWeight: 800, color: '#1C1410', margin: '20px 0 14px', paddingBottom: 8, borderBottom: '1px solid #EAE4DC' }}>بيانات مسؤول المنشأة</div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                            ...
                            <div style={{ marginBottom: 16 }}>
                                ...
                                <input className="rc-inp" type="text" placeholder="مدير المنشأة" value={data.contact_title} onChange={e => setData('contact_title', e.target.value)} />
                            </div>
                        </div>

                        <div style={{ marginBottom: 16 }}>
                            <label style={{ display: 'block', fontSize: 12, color: '#4A5C78', fontWeight: 600, marginBottom: 6 }}>البريد الإلكتروني</label>
                            <input className="rc-inp" type="email" dir="ltr" placeholder="info@business.com" value={data.email} onChange={e => setData('email', e.target.value)} />
                        </div>
                        ...
                        <div style={{ textAlign: 'center', fontSize: 13, color: '#7A8BA8' }}>
                            لديك حساب؟ <a href="/business/login" style={{ color: '#C8410A', fontWeight: 700, textDecoration: 'none' }}>سجّل دخولك</a>
                        </div>
                    </form>
                </div>
            </div>
