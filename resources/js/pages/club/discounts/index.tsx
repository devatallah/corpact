...
import type { Company, Community, Discount, Sport } from '@/types/models';
...
export default function DiscountsIndex({ discounts, companies }: Props) {
    ...
    const [communities, setCommunities] = useState<CommunityWithSport[]>([]);
    ...
    useEffect(() => {
        ...
        fetch(`/club/discounts/communities/${companyId}`, {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (editingItem) {
            form.put('/club/discounts/' + editingItem.id, {
                onSuccess: () => {
                    setEditingItem(null);
    ...
    const handleDelete = (discount: DiscountWithRelations) => {
        ...
        router.delete('/club/discounts/' + discount.id, {
                                ...
                                <div className="frow">
                                    ...
                                    <div className="fg">
                                        ...
                                        >
                                            ...
                                            {communities.map((c) => (
                                                <option key={c.id} value={c.id}>{c.name} ({c.sport?.name})</option>
                                            ))}
                                        </select>
