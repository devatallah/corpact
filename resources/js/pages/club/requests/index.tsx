...
import type { Club, Event, EventAlternative, PaginatedResult } from '@/types/models';
    ...
    return (
        <div className="card" style={{ borderRight: `4px solid ${statusBorderColor(event.status)}`, marginBottom: 16 }}>
            ...
            <div className="req-grid">
                ...
                <div className="ri">
                    <div className="rl">🏟️ عدد الملاعب</div>
                    <div className="rv">{event.courts_count} {event.courts_count > 1 ? 'ملاعب' : 'ملعب'}</div>
                </div>
                <div className="ri">
                ...
                <div className="ri">
                    ...
                    <div className="rv" style={{ color: '#8A7868', fontSize: 11 }}>
                        {event.courts_count} ملعب &times; {event.courts_count > 0 ? Math.round(event.total_amount / event.courts_count).toLocaleString() : 0} ر
                    </div>
                </div>
            </div>
                    ...
                    <div style={{ background: 'rgba(26,95,171,.06)', border: '1px solid rgba(26,95,171,.25)', borderRadius: 10, padding: '12px 16px', marginTop: 10 }}>
                        ...
                        <div className="req-grid">
                            ...
                            {alt.proposed_courts_count && (
                                <div className="ri">
                                    <div className="rl">🏟️ ملاعب</div>
                                    <div className="rv">{alt.proposed_courts_count}</div>
                                </div>
                            )}
                            {alt.proposed_amount && (
                                <div className="ri">
            ...
            {/* Alternative form */}
            {event.status === 'waiting_club' && showAlt && (
            ...
            {/* Action buttons */}
            {event.status === 'waiting_club' && !showAlt && !showRejectDialog && (
                <div style={{ display: 'flex', gap: 10 }}>
                    <button
                        className="act-btn btn-reject"
                        style={{ flex: 1 }}
...
export default function BookingRequests({ club, events, filters, pendingCount }: Props) {
    return (
        <ClubLayout>
