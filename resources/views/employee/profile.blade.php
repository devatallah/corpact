  ...
  <div id="ptE">
    ...
    <div onclick="window.location='{{ route('employee.events.show', $event) }}'" class="card" style="cursor:pointer;border-right:3px solid {{ $eColor }};">
      <div style="display:flex;justify-content:space-between;margin-bottom:8px;">
        <div><div style="font-size:13px;font-weight:700;">{{ $event->partner->name }}</div><div style="font-size:11px;color:#7A8BA8;">{{ $event->community->name }}</div></div>
        <span class="badge" style="background:{{ $event->status === 'confirmed' ? '#009E8218' : '#D4820A18' }};color:{{ $event->status === 'confirmed' ? '#009E82' : '#D4820A' }};">{{ $event->status === 'confirmed' ? 'مؤكدة' : ($event->status === 'open' ? 'مفتوح' : ($event->status === 'waiting_partner' ? 'انتظار الشريك' : $event->status)) }}</span>
      </div>
