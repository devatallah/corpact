...
<div class="screen on" id="sCommunity">
  ...
  <!-- Events Tab -->
  <div id="ctE">
    @forelse($events ?? $selected->events()->with(['partner','sport'])->whereIn('status', ['open','full','confirmed'])->latest('event_date')->get() as $event)
    <div onclick="window.location='{{ route('employee.events.show', $event) }}'" class="card" style="cursor:pointer;border-right:3px solid {{ $color }};">
      <div style="font-size:13px;font-weight:700;">{{ $event->partner->name }}</div>
