...
<div class="sidebar" id="sb">
  ...
  <nav>
    ...
    <div class="ni on" onclick="window.location='/admin/partners'"><span>🏟️</span><span class="nl">الشركاء</span></div>
...
<div class="sc on">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:24px;">
    <a href="{{ route('admin.partners.index') }}" style="color:#6B7A99;text-decoration:none;font-size:14px;">← الشركاء</a>
    <span style="color:#3D4A60;">/</span>
    <span style="color:#fff;font-weight:700;">تعديل: {{ $partner->name }}</span>
  </div>
  ...
  <div class="card" style="max-width:600px;">
    <div style="font-size:18px;font-weight:700;margin-bottom:20px;">تعديل بيانات الشريك</div>
    <form method="POST" action="{{ route('admin.partners.update', $partner) }}">
      @csrf
      @method('PUT')

      <div style="margin-bottom:16px;">
        <label style="display:block;font-size:12px;font-weight:600;color:rgba(255,255,255,.6);margin-bottom:6px;">اسم الشريك</label>
        <input type="text" name="name" value="{{ old('name', $partner->name) }}" style="width:100%;padding:10px 14px;background:#0F1117;border:1px solid #232A3E;border-radius:10px;color:#E8EAF0;font-size:14px;font-family:inherit;outline:none;">
      </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px;">
        <div>
          ...
          <input type="text" name="city" value="{{ old('city', $partner->city) }}" style="width:100%;padding:10px 14px;background:#0F1117;border:1px solid #232A3E;border-radius:10px;color:#E8EAF0;font-size:14px;font-family:inherit;outline:none;">
        </div>
        <div>
          <label style="display:block;font-size:12px;font-weight:600;color:rgba(255,255,255,.6);margin-bottom:6px;">الحي</label>
          <input type="text" name="district" value="{{ old('district', $partner->district) }}" style="width:100%;padding:10px 14px;background:#0F1117;border:1px solid #232A3E;border-radius:10px;color:#E8EAF0;font-size:14px;font-family:inherit;outline:none;">
        </div>
      </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px;">
        <div>
          ...
          <input type="email" name="email" value="{{ old('email', $partner->email) }}" dir="ltr" style="width:100%;padding:10px 14px;background:#0F1117;border:1px solid #232A3E;border-radius:10px;color:#E8EAF0;font-size:14px;font-family:inherit;outline:none;">
        </div>
        <div>
          <label style="display:block;font-size:12px;font-weight:600;color:rgba(255,255,255,.6);margin-bottom:6px;">رقم الهاتف</label>
          <input type="text" name="contact_phone" value="{{ old('contact_phone', $partner->contact_phone) }}" dir="ltr" style="width:100%;padding:10px 14px;background:#0F1117;border:1px solid #232A3E;border-radius:10px;color:#E8EAF0;font-size:14px;font-family:inherit;outline:none;">
        </div>
      </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px;">
        <div>
          ...
          <input type="number" name="commission_rate" value="{{ old('commission_rate', $partner->commission_rate) }}" min="0" max="100" step="0.1" style="width:100%;padding:10px 14px;background:#0F1117;border:1px solid #232A3E;border-radius:10px;color:#E8EAF0;font-size:14px;font-family:inherit;outline:none;">
        </div>
        <div>
          ...
          <select name="status" style="width:100%;padding:10px 14px;background:#0F1117;border:1px solid #232A3E;border-radius:10px;color:#E8EAF0;font-size:14px;font-family:inherit;outline:none;">
            <option value="pending" {{ old('status', $partner->status) === 'pending' ? 'selected' : '' }}>معلق</option>
            <option value="active" {{ old('status', $partner->status) === 'active' ? 'selected' : '' }}>نشط</option>
            <option value="rejected" {{ old('status', $partner->status) === 'rejected' ? 'selected' : '' }}>مرفوض</option>
            <option value="suspended" {{ old('status', $partner->status) === 'suspended' ? 'selected' : '' }}>معلّق</option>
          </select>
        </div>
      </div>
      ...
      <div style="display:flex;gap:10px;margin-top:24px;">
        ...
        <a href="{{ route('admin.partners.index') }}" style="padding:12px 24px;background:#232A3E;border-radius:10px;color:#6B7A99;font-size:14px;font-weight:700;text-decoration:none;text-align:center;">إلغاء</a>
      </div>
    </form>
  </div>
</div>
