...
<title>تيمات — تعديل مزود خدمة</title>
...
<div class="sc on">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:24px;">
    ...
    <span style="color:#fff;font-weight:700;">تعديل: {{ $business->name }}</span>
  </div>
  ...
  <div class="card" style="max-width:600px;">
    <div style="font-size:18px;font-weight:700;margin-bottom:20px;">تعديل بيانات مزود الخدمة</div>
    <form method="POST" action="{{ route('admin.businesss.update', $business) }}">
      @csrf
      @method('PUT')
      ...
      <div style="margin-bottom:16px;">
        ...
        <input type="text" name="name" value="{{ old('name', $business->name) }}" style="width:100%;padding:10px 14px;background:#0F1117;border:1px solid #232A3E;border-radius:10px;color:#E8EAF0;font-size:14px;font-family:inherit;outline:none;">
      </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px;">
        <div>
          ...
          <input type="text" name="city" value="{{ old('city', $business->city) }}" style="width:100%;padding:10px 14px;background:#0F1117;border:1px solid #232A3E;border-radius:10px;color:#E8EAF0;font-size:14px;font-family:inherit;outline:none;">
        </div>
        <div>
          <label style="display:block;font-size:12px;font-weight:600;color:rgba(255,255,255,.6);margin-bottom:6px;">الحي</label>
          <input type="text" name="district" value="{{ old('district', $business->district) }}" style="width:100%;padding:10px 14px;background:#0F1117;border:1px solid #232A3E;border-radius:10px;color:#E8EAF0;font-size:14px;font-family:inherit;outline:none;">
        </div>
      </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px;">
        <div>
          ...
          <input type="email" name="email" value="{{ old('email', $business->email) }}" dir="ltr" style="width:100%;padding:10px 14px;background:#0F1117;border:1px solid #232A3E;border-radius:10px;color:#E8EAF0;font-size:14px;font-family:inherit;outline:none;">
        </div>
        <div>
          <label style="display:block;font-size:12px;font-weight:600;color:rgba(255,255,255,.6);margin-bottom:6px;">رقم الهاتف</label>
          <input type="text" name="contact_phone" value="{{ old('contact_phone', $business->contact_phone) }}" dir="ltr" style="width:100%;padding:10px 14px;background:#0F1117;border:1px solid #232A3E;border-radius:10px;color:#E8EAF0;font-size:14px;font-family:inherit;outline:none;">
        </div>
      </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px;">
        <div>
          ...
          <input type="number" name="commission_rate" value="{{ old('commission_rate', $business->commission_rate) }}" min="0" max="100" step="0.1" style="width:100%;padding:10px 14px;background:#0F1117;border:1px solid #232A3E;border-radius:10px;color:#E8EAF0;font-size:14px;font-family:inherit;outline:none;">
        </div>
        <div>
          ...
          <select name="status" style="width:100%;padding:10px 14px;background:#0F1117;border:1px solid #232A3E;border-radius:10px;color:#E8EAF0;font-size:14px;font-family:inherit;outline:none;">
            <option value="pending" {{ old('status', $business->status) === 'pending' ? 'selected' : '' }}>معلق</option>
            <option value="active" {{ old('status', $business->status) === 'active' ? 'selected' : '' }}>نشط</option>
            <option value="rejected" {{ old('status', $business->status) === 'rejected' ? 'selected' : '' }}>مرفوض</option>
            <option value="suspended" {{ old('status', $business->status) === 'suspended' ? 'selected' : '' }}>معلّق</option>
          </select>
        </div>
      </div>
