...
<title>تيمات — إدارة الشركاء</title>
...
<div class="sidebar" id="sb">
  ...
  <nav>
    ...
    <div class="ni on" onclick="window.location='/admin/partners'"><span>🏟️</span><span class="nl">الشركاء</span>@if($stats['pending'] > 0)<span class="nb">{{ $stats['pending'] }}</span>@endif</div>
...
<div class="sc on" id="partners">
  <div class="page-title">إدارة الشركاء</div>
  <div class="page-sub">{{ $stats['active'] }} شركاء مفعّلين · {{ $stats['pending'] }} طلبات معلقة</div>
  ...
  <div class="card" style="padding:0;overflow:hidden;">
    <table>
      <thead><tr><th>الشريك</th><th>المدينة</th><th>الرياضات</th><th>الملاعب</th><th>مسؤول الشريك</th><th>الحالة</th><th>إجراء</th></tr></thead>
      <tbody>
        @forelse($partners as $partner)
        <tr class="cl-row" data-s="@if($partner->status === 'pending')معلق @elseif($partner->status === 'active')نشط @elseif($partner->status === 'rejected')مرفوض @else معلق @endif">
          <td><div style="font-weight:700;color:#fff;">{{ $partner->name }}</div><div style="font-size:10px;color:#6B7A99;">@if($partner->status === 'active'){{ $partner->district }}@else{{ $partner->created_at->diffForHumans() }}@endif</div></td>
          <td style="color:#C8D0E0;">{{ $partner->city }}</td>
          <td><span style="font-size:12px;">@if($partner->sports && $partner->sports->count()){{ $partner->sports->map(fn($s) => ($s->icon ?? '') . ' ' . $s->name)->implode(' · ') }}@else -@endif</span></td><td>{{ $partner->venues_count ?? $partner->venues()->count() }}</td>
          <td><div style="font-size:12px;">{{ $partner->email ?? '-' }}</div><div style="font-size:10px;color:#6B7A99;">{{ $partner->contact_phone ?? '-' }}</div></td>
          <td>
          ...
          <td>
            ...
            <div style="display:flex;gap:6px;">
              <form method="POST" action="{{ route('admin.partners.approve', $partner) }}" style="margin:0;">@csrf<button type="submit" class="act-btn btn-approve">قبول</button></form>
              <form method="POST" action="{{ route('admin.partners.reject', $partner) }}" style="margin:0;">@csrf<button type="submit" class="act-btn btn-reject">رفض</button></form>
              <button class="act-btn btn-view" onclick="openPanel('partner','{{ $partner->name }}')">تفاصيل</button>
            </div>
            @else
            <button class="act-btn btn-view" onclick="openPanel('partner','{{ $partner->name }}')">تفاصيل</button>
            @endif
        ...
        @empty
        <tr><td colspan="7" style="text-align:center;color:#6B7A99;padding:20px;">لا يوجد شركاء</td></tr>
        @endforelse
      </tbody>
    </table>
  </div>
...
var panelData={
  partner:{
    ...
    '{{ $cl->name }}':{id:{{ $cl->id }},rows:[['اسم الشريك','{{ $cl->name }}'],['المدينة','{{ $cl->city }}@if($cl->district) · {{ $cl->district }}@endif'],['الرياضات','{{ $cl->sports?->map(fn($s) => ($s->icon ?? "") . " " . $s->name)->implode(" · ") ?? "-" }}'],['عدد الملاعب','{{ $cl->venues_count ?? $cl->venues()->count() }}'],['ساعات العمل','{{ $cl->working_hours ?? "-" }}'],['البريد','{{ $cl->email ?? "-" }}'],['جوال التواصل','{{ $cl->contact_phone ?? "-" }}'],@if($cl->status === 'active')['تاريخ التفعيل','{{ $cl->approved_at?->format("j F Y") }}'],['الحالة','نشط ✅']@else['تاريخ الطلب','{{ $cl->created_at->diffForHumans() }}']@endif]},
    @endforeach
  }
};
...
function doApprove(){
  submitAction('/admin/partners/'+currentPanelId+'/approve');
}
function doReject(){
  submitAction('/admin/partners/'+currentPanelId+'/reject');
}
