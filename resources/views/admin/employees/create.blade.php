...
<div class="sc on">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:24px;">
    <a href="{{ route('admin.employees.index') }}" style="color:#6B7A99;text-decoration:none;font-size:14px;">← الموظفون</a>
    <span style="color:#3D4A60;">/</span>
    <span style="color:#fff;font-weight:700;">إضافة موظف</span>
  </div>
  ...
  <div class="card" style="max-width:600px;">
    <div style="font-size:18px;font-weight:700;margin-bottom:20px;">إضافة موظف جديد</div>
    <form method="POST" action="{{ route('admin.employees.store') }}">
      @csrf

      <div style="margin-bottom:16px;">
        <label style="display:block;font-size:12px;font-weight:600;color:rgba(255,255,255,.6);margin-bottom:6px;">معرّف المستخدم *</label>
        <input type="number" name="user_id" value="{{ old('user_id') }}" required min="1" style="width:100%;padding:10px 14px;background:#0F1117;border:1px solid #232A3E;border-radius:10px;color:#E8EAF0;font-size:14px;font-family:inherit;outline:none;" placeholder="رقم المستخدم">
      </div>

      <div style="margin-bottom:16px;">
        <label style="display:block;font-size:12px;font-weight:600;color:rgba(255,255,255,.6);margin-bottom:6px;">معرّف الشركة *</label>
        <input type="number" name="company_id" value="{{ old('company_id') }}" required min="1" style="width:100%;padding:10px 14px;background:#0F1117;border:1px solid #232A3E;border-radius:10px;color:#E8EAF0;font-size:14px;font-family:inherit;outline:none;" placeholder="رقم الشركة">
      </div>

      <div style="margin-bottom:24px;">
        <label style="display:block;font-size:12px;font-weight:600;color:rgba(255,255,255,.6);margin-bottom:6px;">القسم</label>
        <input type="text" name="department" value="{{ old('department') }}" style="width:100%;padding:10px 14px;background:#0F1117;border:1px solid #232A3E;border-radius:10px;color:#E8EAF0;font-size:14px;font-family:inherit;outline:none;" placeholder="مثال: الموارد البشرية">
      </div>
      ...
      <div style="display:flex;gap:10px;">
        ...
        <a href="{{ route('admin.employees.index') }}" style="padding:12px 24px;background:#232A3E;border-radius:10px;color:#6B7A99;font-size:14px;font-weight:700;text-decoration:none;text-align:center;">إلغاء</a>
      </div>
    </form>
  </div>
</div>
