...
<div class="sc on">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:24px;">
    ...
    <span style="color:#fff;font-weight:700;">تعديل: {{ $employee->name }}</span>
  </div>
  ...
  <div class="card" style="max-width:600px;">
    <div style="font-size:18px;font-weight:700;margin-bottom:6px;">تعديل بيانات الموظف</div>
    <div style="font-size:13px;color:#6B7A99;margin-bottom:20px;">{{ $employee->email }} — {{ $employee->company->name ?? '-' }}</div>
    <form method="POST" action="{{ route('admin.employees.update', $employee) }}">
      ...
      <div style="margin-bottom:16px;">
        ...
        <input type="text" name="department" value="{{ old('department', $employee->department) }}" style="width:100%;padding:10px 14px;background:#0F1117;border:1px solid #232A3E;border-radius:10px;color:#E8EAF0;font-size:14px;font-family:inherit;outline:none;" placeholder="القسم">
      </div>
