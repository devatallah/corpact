...
<div class="sidebar" id="sb">
  ...
  <nav>
    ...
    <div class="ni" onclick="window.location='/admin/partners'"><span>🏟️</span><span class="nl">الشركاء</span></div>
...
<div class="sc on">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:24px;">
    ...
    <span style="color:#fff;font-weight:700;">تعديل: {{ $company->name }}</span>
  </div>
  ...
  <div class="card" style="max-width:600px;">
    <div style="font-size:18px;font-weight:700;margin-bottom:20px;">تعديل بيانات الشركة</div>
    <form method="POST" action="{{ route('admin.companies.update', $company) }}">
      ...
      <div style="margin-bottom:16px;">
        ...
        <input type="text" name="name" value="{{ old('name', $company->name) }}" required style="width:100%;padding:10px 14px;background:#0F1117;border:1px solid #232A3E;border-radius:10px;color:#E8EAF0;font-size:14px;font-family:inherit;outline:none;">
      </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px;">
        <div>
          ...
          <input type="text" name="domain" value="{{ old('domain', $company->domain) }}" dir="ltr" style="width:100%;padding:10px 14px;background:#0F1117;border:1px solid #232A3E;border-radius:10px;color:#E8EAF0;font-size:14px;font-family:inherit;outline:none;">
        </div>
        <div>
          <label style="display:block;font-size:12px;font-weight:600;color:rgba(255,255,255,.6);margin-bottom:6px;">القطاع</label>
          <input type="text" name="sector" value="{{ old('sector', $company->sector) }}" style="width:100%;padding:10px 14px;background:#0F1117;border:1px solid #232A3E;border-radius:10px;color:#E8EAF0;font-size:14px;font-family:inherit;outline:none;">
        </div>
      </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px;">
        <div>
          ...
          <input type="number" name="employee_count" value="{{ old('employee_count', $company->employee_count) }}" min="1" style="width:100%;padding:10px 14px;background:#0F1117;border:1px solid #232A3E;border-radius:10px;color:#E8EAF0;font-size:14px;font-family:inherit;outline:none;">
        </div>
        <div>
          <label style="display:block;font-size:12px;font-weight:600;color:rgba(255,255,255,.6);margin-bottom:6px;">المدينة</label>
          <input type="text" name="city" value="{{ old('city', $company->city) }}" style="width:100%;padding:10px 14px;background:#0F1117;border:1px solid #232A3E;border-radius:10px;color:#E8EAF0;font-size:14px;font-family:inherit;outline:none;">
        </div>
      </div>
      ...
      <div style="margin-bottom:24px;">
        ...
        <select name="status" style="width:100%;padding:10px 14px;background:#0F1117;border:1px solid #232A3E;border-radius:10px;color:#E8EAF0;font-size:14px;font-family:inherit;outline:none;">
          <option value="pending" {{ old('status', $company->status) === 'pending' ? 'selected' : '' }}>معلق</option>
          <option value="review" {{ old('status', $company->status) === 'review' ? 'selected' : '' }}>قيد المراجعة</option>
          <option value="active" {{ old('status', $company->status) === 'active' ? 'selected' : '' }}>نشط</option>
          <option value="rejected" {{ old('status', $company->status) === 'rejected' ? 'selected' : '' }}>مرفوض</option>
        </select>
      </div>
