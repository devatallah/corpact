...
<div class="sc on">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:24px;">
    ...
    <span style="color:#fff;font-weight:700;">تعديل فعالية #{{ $event->id }}</span>
  </div>
  ...
  <div class="card" style="max-width:600px;">
    ...
    <div style="font-size:13px;color:#6B7A99;margin-bottom:20px;">{{ $event->community?->name ?? '-' }} — {{ $event->partner?->name ?? '-' }} — {{ $event->sport?->name ?? '-' }}</div>
    <form method="POST" action="{{ route('admin.events.update', $event) }}">
      ...
      <div style="margin-bottom:16px;">
        ...
        <input type="date" name="date" value="{{ old('date', $event->date?->format('Y-m-d') ?? $event->date) }}" dir="ltr" style="width:100%;padding:10px 14px;background:#0F1117;border:1px solid #232A3E;border-radius:10px;color:#E8EAF0;font-size:14px;font-family:inherit;outline:none;">
      </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px;">
        <div>
          ...
          <input type="time" name="start_time" value="{{ old('start_time', $event->start_time) }}" dir="ltr" style="width:100%;padding:10px 14px;background:#0F1117;border:1px solid #232A3E;border-radius:10px;color:#E8EAF0;font-size:14px;font-family:inherit;outline:none;">
        </div>
        <div>
          <label style="display:block;font-size:12px;font-weight:600;color:rgba(255,255,255,.6);margin-bottom:6px;">وقت النهاية</label>
          <input type="time" name="end_time" value="{{ old('end_time', $event->end_time) }}" dir="ltr" style="width:100%;padding:10px 14px;background:#0F1117;border:1px solid #232A3E;border-radius:10px;color:#E8EAF0;font-size:14px;font-family:inherit;outline:none;">
        </div>
      </div>
      ...
      <div style="margin-bottom:24px;">
        ...
        <select name="status" style="width:100%;padding:10px 14px;background:#0F1117;border:1px solid #232A3E;border-radius:10px;color:#E8EAF0;font-size:14px;font-family:inherit;outline:none;">
          ...
          <option value="waiting_partner" {{ old('status', $event->status) === 'waiting_partner' ? 'selected' : '' }}>بانتظار الشريك</option>
