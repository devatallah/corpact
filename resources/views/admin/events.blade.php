...
<div class="sc on" id="events">
  ...
  <div class="card" style="padding:0;overflow:hidden;">
    <table>
      <thead><tr><th>الفعالية</th><th>الشركة</th><th>الشريك</th><th>التاريخ</th><th>اللاعبون</th><th>المبلغ</th><th>الحالة</th></tr></thead>
      <tbody>
        @forelse($events as $event)
        @php
          $filterStatus = match($event->status) {
            'open', 'waiting_partner', 'alternative_proposed' => 'مفتوحة',
        ...
        @endphp
        <tr class="ev-row" data-s="{{ $filterStatus }}"><td><span style="font-weight:600;color:#fff;">{{ $event->sport?->icon ?? '' }} {{ $event->sport?->name ?? '-' }}</span></td><td style="color:#C8D0E0;">{{ $event->company?->name ?? ($event->community?->company?->name ?? '-') }}</td><td style="color:#C8D0E0;">{{ $event->partner?->name ?? '-' }}</td><td style="font-size:12px;color:#6B7A99;">{{ $event->event_date?->translatedFormat('l j F') }} · {{ $event->start_time }}</td><td>{{ $event->participants_count }}/{{ $event->capacity }}</td><td style="color:@if(in_array($event->status, ['completed']))#009E82 @else #D4820A @endif;font-weight:700;">{{ number_format($event->total_amount) }} ر</td><td>
          @if($event->status === 'open')
          <span class="badge b-review">🔍 مفتوحة</span>
          @elseif($event->status === 'waiting_partner')
          <span class="badge b-pending">انتظار الشريك</span>
