...
<title>كورباكت — إدارة الملاعب</title>
...
<div class="sc on" id="ccourts">
  ...
  <div id="cList">
    ...
    @php
      $sportIcon = $court->sport?->icon ?? '🏟️';
      $sportName = $court->sport?->name ?? '';
      $statusBadge = match($court->status) {
        ...
        default => ['bg' => '#8A786818', 'color' => '#8A7868', 'label' => $court->status],
      };
      $pricingSummary = $court->pricings->map(fn($p) => $p->duration_minutes . ' د: ' . number_format($p->price) . ' ر')->join(' · ');
      if (!$pricingSummary) {
        $pricingSummary = 'لم يُحدد سعر';
      }
    @endphp
    <div class="card" style="margin-bottom:14px;" id="ct{{ $court->id }}">
      ...
      @endif
      <div id="e{{ $court->id }}" style="display:none;background:#F7F4F0;border-radius:12px;padding:16px;margin-top:14px;">
        <form action="{{ route('club.courts.update', $court) }}" method="POST">
          @csrf
          @method('PUT')
          <div class="frow"><div class="fg"><label>اسم الملعب</label><input type="text" name="name" value="{{ $court->name }}"></div><div class="fg"><label>نوع الرياضة</label><select name="sport_id">@foreach($sports as $sport)<option value="{{ $sport->id }}" {{ $court->sport_id == $sport->id ? 'selected' : '' }}>{{ $sport->icon }} {{ $sport->name }}</option>@endforeach</select></div></div>
          <div class="frow"><div class="fg"><label>الحالة</label><select name="status"><option value="active" {{ $court->status === 'active' ? 'selected' : '' }}>● نشط</option><option value="closed" {{ $court->status === 'closed' ? 'selected' : '' }}>● مغلق</option><option value="maintenance" {{ $court->status === 'maintenance' ? 'selected' : '' }}>🔧 صيانة</option></select></div></div>
          <div style="display:flex;gap:10px;"><button type="submit" style="flex:2;padding:10px;background:#1A7A4A;color:#fff;border:none;border-radius:10px;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit;">حفظ ✓</button><button type="button" onclick="togEdit('e{{ $court->id }}')" style="flex:1;padding:10px;background:#EAE4DC;color:#8A7868;border:none;border-radius:10px;font-size:13px;cursor:pointer;font-family:inherit;">إلغاء</button></div>
        </form>
      </div>
    </div>
    @empty
