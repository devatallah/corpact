...
<div class="sc on" id="creqs">
  ...
  <div style="margin-bottom:20px;">
    <a href="{{ route('business.bookings.index') }}" class="fbtn {{ !isset($filters['status']) ? 'on' : '' }}">الكل</a>
    <a href="{{ route('business.bookings.index', ['status' => 'waiting_business']) }}" class="fbtn {{ ($filters['status'] ?? '') === 'waiting_business' ? 'on' : '' }}">معلق</a>
    <a href="{{ route('business.bookings.index', ['status' => 'confirmed']) }}" class="fbtn {{ ($filters['status'] ?? '') === 'confirmed' ? 'on' : '' }}">مقبول</a>
    <a href="{{ route('business.bookings.index', ['status' => 'rejected']) }}" class="fbtn {{ ($filters['status'] ?? '') === 'rejected' ? 'on' : '' }}">مرفوض</a>
    <a href="{{ route('business.bookings.index', ['status' => 'alternative_proposed']) }}" class="fbtn {{ ($filters['status'] ?? '') === 'alternative_proposed' ? 'on' : '' }}">وقت بديل</a>
  </div>
  <div id="rList">
    ...
    <div class="card req" data-s="{{ $statusAr }}" style="border-right:4px solid {{ $statusColor }};margin-bottom:16px;">
      ...
      <div style="display:flex;gap:10px;" id="ra{{ $event->id }}">
        <form action="{{ route('business.bookings.reject', $event) }}" method="POST" style="flex:1;" onsubmit="return confirm('هل أنت متأكد من رفض هذا الطلب؟')">
          @csrf
        ...
        <form action="{{ route('business.bookings.approve', $event) }}" method="POST" style="flex:2;">
          @csrf
