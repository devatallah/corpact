...
<div class="sidebar" id="sb">
  ...
  <nav>
    ...
    <div class="ni on" onclick="window.location='/club/schedule'"><span>📅</span><span class="nl"> التقويم</span></div>
...
@php
  ...
  $courts = $schedule['courts'] ?? collect();
  ...
  // Build a lookup: courtId -> time -> event data
  $bookingLookup = [];
  foreach ($grid as $courtData) {
    $courtId = $courtData['court_id'];
    foreach ($courtData['events'] as $event) {
      $time = \Carbon\Carbon::parse($event['start_time'])->format('H:i');
      $bookingLookup[$courtId][$time] = $event;
    }
  }
@endphp
<div class="sc on" id="csched">
  ...
  <div style="background:#fff;border:1px solid #EAE4DC;border-radius:16px;overflow:auto;">
    <table style="min-width:520px;">
      <thead><tr><th style="background:#F7F4F0;">الوقت</th>@foreach($courts as $court)<th style="background:#F7F4F0;">{{ $court->name }}</th>@endforeach</tr></thead>
      <tbody>
        ...
        <tr>
          ...
          @php $booking = $bookingLookup[$court->id][$slot] ?? null; @endphp
          @if($booking)
