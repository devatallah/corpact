...
<div class="sc on">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:24px;">
    ...
    <span style="font-weight:700;">تعديل: {{ $employee->name }}</span>
  </div>
  ...
  <div style="background:#fff;border:1px solid #E2E8F4;border-radius:16px;padding:32px;max-width:500px;">
    ...
    <div style="font-size:13px;color:#7A8BA8;margin-bottom:20px;">{{ $employee->email }}</div>
    <form method="POST" action="{{ route('company.employees.update', $employee) }}">
      @csrf
      @method('PUT')

      <div style="margin-bottom:16px;">
        <label style="display:block;font-size:12px;font-weight:600;color:#4A5C78;margin-bottom:6px;">القسم</label>
        <input type="text" name="department" value="{{ old('department', $employee->department) }}" style="width:100%;padding:10px 14px;background:#F0F2F8;border:1px solid #E2E8F4;border-radius:10px;font-size:14px;font-family:inherit;outline:none;" placeholder="القسم">
      </div>
