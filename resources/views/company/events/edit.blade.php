...
<div class="sc on">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:24px;">
    ...
    <span style="font-weight:700;">تعديل فعالية #{{ $event->id }}</span>
  </div>
  ...
  <div style="background:#fff;border:1px solid #E2E8F4;border-radius:16px;padding:32px;max-width:600px;">
    ...
    <div style="font-size:13px;color:#7A8BA8;margin-bottom:20px;">{{ $event->community?->name ?? '-' }} — {{ $event->partner?->name ?? '-' }} — {{ $event->sport?->name ?? '-' }}</div>
    <form method="POST" action="{{ route('company.events.update', $event) }}">
      @csrf
      @method('PUT')

      <div style="margin-bottom:16px;">
        <label style="display:block;font-size:12px;font-weight:600;color:#4A5C78;margin-bottom:6px;">التاريخ</label>
        <input type="date" name="date" value="{{ old('date', $event->date?->format('Y-m-d') ?? $event->date) }}" dir="ltr" style="width:100%;padding:10px 14px;background:#F0F2F8;border:1px solid #E2E8F4;border-radius:10px;font-size:14px;font-family:inherit;outline:none;">
      </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px;">
        <div>
          ...
          <input type="time" name="start_time" value="{{ old('start_time', $event->start_time) }}" dir="ltr" style="width:100%;padding:10px 14px;background:#F0F2F8;border:1px solid #E2E8F4;border-radius:10px;font-size:14px;font-family:inherit;outline:none;">
        </div>
        <div>
          <label style="display:block;font-size:12px;font-weight:600;color:#4A5C78;margin-bottom:6px;">وقت النهاية</label>
          <input type="time" name="end_time" value="{{ old('end_time', $event->end_time) }}" dir="ltr" style="width:100%;padding:10px 14px;background:#F0F2F8;border:1px solid #E2E8F4;border-radius:10px;font-size:14px;font-family:inherit;outline:none;">
        </div>
      </div>
