...
<div class="sidebar" id="sb">
  ...
  <nav>
    <div class="ni on" onclick="window.location='/partner/dash'"><span>📊</span><span class="nl"> الرئيسية</span></div>
    <div class="ni" onclick="window.location='/partner/requests'"><span>📋</span><span class="nl"> طلبات الحجز</span>@if($stats['pending_requests'] > 0)<span class="nl" style="background:#C8410A;color:#fff;font-size:10px;padding:1px 7px;border-radius:20px;margin-right:auto;">{{ $stats['pending_requests'] }}</span>@endif</div>
...
<div class="sc on" id="cdash">
  <div style="background:linear-gradient(135deg,#1C1410,#2A1F18);border-radius:20px;padding:24px 28px;margin-bottom:24px;color:#fff;display:flex;justify-content:space-between;align-items:center;">
    <div><div style="font-size:11px;color:rgba(255,255,255,.4);letter-spacing:2px;margin-bottom:4px;">TEAMAT · partner PORTAL</div><div style="font-size:24px;font-weight:900;">{{ $partner->name }}</div><div style="font-size:13px;color:rgba(255,255,255,.5);margin-top:4px;">{{ $partner->district }}، {{ $partner->city }}</div></div>
    <div style="text-align:left;"><div style="font-size:32px;font-weight:900;color:#F5A623;">{{ $partner->rating ?? '0.0' }} ⭐</div><div style="font-size:11px;color:rgba(255,255,255,.4);">{{ number_format($partner->total_bookings ?? 0) }} حجز إجمالي</div></div>
  </div>
  <div class="stat-row">
  ...
  <div class="card">
    <div style="font-size:15px;font-weight:800;margin-bottom:14px;display:flex;justify-content:space-between;">
      طلبات تحتاج ردك
      <button onclick="window.location='/partner/requests'" style="background:#C8410A18;color:#C8410A;border:none;border-radius:10px;padding:6px 14px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit;">عرض الكل</button>
    </div>
    @forelse($pendingEvents as $event)
    <div onclick="window.location='/partner/requests'" style="background:#F7F4F0;border:1px solid #EAE4DC;border-right:3px solid #C8410A;border-radius:12px;padding:12px 14px;cursor:pointer;{{ !$loop->last ? 'margin-bottom:10px;' : '' }}">
