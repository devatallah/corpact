...
<div class="sidebar" id="sb">
  ...
  <div class="co-info"><div class="lbl">الشريك</div><div class="nm">{{ $partner->name }}</div><div class="sb">{{ $partner->district }}، {{ $partner->city }}</div></div>
  <nav>
    <div class="ni" onclick="window.location='/partner/dash'"><span>📊</span><span class="nl"> الرئيسية</span></div>
    <div class="ni on" onclick="window.location='/partner/requests'"><span>📋</span><span class="nl"> طلبات الحجز</span>@if($pendingCount > 0)<span class="nl" id="pBadge" style="background:#C8410A;color:#fff;font-size:10px;padding:1px 7px;border-radius:20px;margin-right:auto;">{{ $pendingCount }}</span>@endif</div>
    <div class="ni" onclick="window.location='/partner/schedule'"><span>📅</span><span class="nl"> التقويم</span></div>
    <div class="ni" onclick="window.location='/partner/venues'"><span>🏟️</span><span class="nl"> إدارة الملاعب</span></div>
...
<div class="sc on" id="creqs">
  ...
  <div style="margin-bottom:20px;">
    <a href="{{ route('partner.bookings.index') }}" class="fbtn {{ !isset($filters['status']) ? 'on' : '' }}">الكل</a>
    <a href="{{ route('partner.bookings.index', ['status' => 'waiting_partner']) }}" class="fbtn {{ ($filters['status'] ?? '') === 'waiting_partner' ? 'on' : '' }}">معلق</a>
    <a href="{{ route('partner.bookings.index', ['status' => 'confirmed']) }}" class="fbtn {{ ($filters['status'] ?? '') === 'confirmed' ? 'on' : '' }}">مقبول</a>
    <a href="{{ route('partner.bookings.index', ['status' => 'rejected']) }}" class="fbtn {{ ($filters['status'] ?? '') === 'rejected' ? 'on' : '' }}">مرفوض</a>
    <a href="{{ route('partner.bookings.index', ['status' => 'alternative_proposed']) }}" class="fbtn {{ ($filters['status'] ?? '') === 'alternative_proposed' ? 'on' : '' }}">وقت بديل</a>
  </div>
  <div id="rList">
    ...
    <div class="card req" data-s="{{ $statusAr }}" style="border-right:4px solid {{ $statusColor }};margin-bottom:16px;">
      ...
      <div class="req-grid">
        ...
        <div class="ri"><div class="rl">🏟️ عدد الملاعب</div><div class="rv">{{ $event->venues_count }} {{ $event->venues_count > 1 ? 'ملاعب' : 'ملعب' }}</div></div>
        ...
        <div class="ri"><div class="rl">📐 الحساب</div><div class="rv" style="color:#8A7868;font-size:11px;">{{ $event->venues_count }} ملعب × {{ $event->venues_count > 0 ? number_format($event->total_amount / $event->venues_count) : 0 }} ر</div></div>
      </div>
      ...
      <div id="af{{ $event->id }}" style="display:none;background:rgba(26,95,171,.06);border:1px solid rgba(26,95,171,.25);border-radius:12px;padding:16px;margin-bottom:14px;">
        ...
        <div class="frow">
          <div class="fg"><label>عدد الملاعب المتاحة</label><select id="ac{{ $event->id }}">@for($i = 1; $i <= max($event->venues_count, 3); $i++)<option value="{{ $i }}" {{ $i == $event->venues_count ? 'selected' : '' }}>{{ $i }} {{ $i > 1 ? 'ملاعب' : 'ملعب' }}</option>@endfor</select></div>
      ...
      <div style="display:flex;gap:10px;" id="ra{{ $event->id }}">
        <form action="{{ route('partner.bookings.reject', $event) }}" method="POST" style="flex:1;" onsubmit="return confirm('هل أنت متأكد من رفض هذا الطلب؟')">
          @csrf
        ...
        <form action="{{ route('partner.bookings.approve', $event) }}" method="POST" style="flex:2;">
          @csrf
