...
export default function AttendancePanel({ eventId, data }: { eventId: number; data: AttendancePanelData }) {
    ...
    return (
        <div className="card" style={{ marginBottom: 16 }}>
            ...
            {data.notice && (
                <div style={{ background: '#FDEDEC', border: '1px solid #FEF9E0', borderRadius: 8, padding: 10, fontSize: 12, marginBottom: 12 }}>
                    ⚠️ {data.notice}
                </div>
            )}

            {data.roster.length === 0 && (
                <div style={{ fontSize: 13, color: 'rgba(10,10,10,.55)' }}>لا مشاركين مؤكدين في هذه الفعالية.</div>
            )}
            ...
            {data.roster.map((row) => {
                ...
                return (
                    <div key={row.employee_id} style={{ borderTop: '1px solid #F6F8F5', padding: '10px 0' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                            ...
                            <span
                                style={{
                                    ...
                                    padding: '3px 10px',
                                    borderRadius: 999,
                                    background: row.attendance_status === 'absent' ? '#FDEDEC' : '#E8F5E9',
                                    color: row.attendance_status === 'absent' ? '#D9381E' : '#2E7D32',
                                }}
                            >
                            ...
                            {data.can_edit && (
                                <>
                                    <input
                                        ...
                                        style={{ padding: '6px 10px', border: '0.5px solid rgba(10,10,10,.15)', borderRadius: 8, fontSize: 12, minWidth: 180 }}
                                    />
                                    <button
                        ...
                        {result ? (
                            <div style={{ fontSize: 12, color: 'rgba(10,10,10,.75)', marginTop: 6, display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                                🏅 النتيجة: <b>{result.value_formatted}</b>
                                {result.corrections_count > 0 && <span style={{ color: 'rgba(10,10,10,.55)' }}>(صُحِّحت {result.corrections_count} مرة)</span>}
                                ...
                                {correcting === result.id && (
                                    <span style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
                                        <input
                                            ...
                                            style={{ padding: '4px 8px', border: '0.5px solid rgba(10,10,10,.15)', borderRadius: 6, width: 90 }}
                                        />
                                        <input
                                            placeholder="سبب التصحيح (إلزامي)"
                                            value={correctionReason}
                                            onChange={(e) => setCorrectionReason(e.target.value)}
                                            style={{ padding: '4px 8px', border: '0.5px solid rgba(10,10,10,.15)', borderRadius: 6, minWidth: 160 }}
                                        />
                            ...
                            data.can_enter_results && row.attendance_status !== 'absent' && (
                                <div style={{ marginTop: 6, display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
                                    <select
                                        ...
                                        style={{ padding: '4px 8px', border: '0.5px solid rgba(10,10,10,.15)', borderRadius: 6, fontSize: 12 }}
                                    >
                                        {data.units.map((u) => (
                                    ...
                                    <input
                                        ...
                                        style={{ padding: '4px 8px', border: '0.5px solid rgba(10,10,10,.15)', borderRadius: 6, width: 90, fontSize: 12 }}
                                    />
