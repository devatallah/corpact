    ...
    return (
        <AdminLayout>
            ...
            <div className="card" style={{ padding: 0, overflowX: 'auto', marginTop: 16 }}>
                <div style={{ padding: '12px 16px', fontWeight: 700, color: '#0A0A0A' }}>سجل الانتقالات</div>
                    ...
                    <tbody>
                        {statusHistory.length === 0 ? (
                            ...
                            statusHistory.map((row) => (
                                <tr key={row.id}>
                                    <td style={{ fontSize: 12, color: 'rgba(10,10,10,.55)', whiteSpace: 'nowrap' }}>
                                        {row.created_at ? fmtDateTime(row.created_at) : '—'}
                                    </td>
                                    <td style={{ color: 'rgba(10,10,10,.55)' }}>{row.from_status ?? '—'}</td>
                                    <td style={{ color: '#0A0A0A', fontWeight: 700 }}>{row.to_status}</td>
                                    <td style={{ color: row.is_manual ? '#C87D00' : 'rgba(10,10,10,.55)', fontWeight: 700 }}>
                                        {row.is_manual ? 'يدوي' : 'آلي'}
                                    </td>
                                    <td style={{ fontSize: 12, color: '#0A0A0A' }}>{row.reason ?? '—'}</td>
                                </tr>
                            ))
                        )}
                    </tbody>
            ...
            <div className="card" style={{ padding: 0, overflowX: 'auto', marginTop: 16 }}>
                <div style={{ padding: '12px 16px', fontWeight: 700, color: '#0A0A0A' }}>
                        ...
                        {notificationLogs.length === 0 ? (
                            ...
                            notificationLogs.map((log) => (
                                <tr key={log.id}>
                                    ...
                                    <td dir="ltr" style={{ fontSize: 12, color: '#0A0A0A' }}>{log.template_key}</td>
                                    <td style={{ fontSize: 12 }}>{log.channel}</td>
                                    <td style={{ fontSize: 12 }}>{log.status}</td>
                                    <td style={{ fontSize: 12, color: 'rgba(10,10,10,.55)' }}>{log.reason ?? '—'}</td>
                                </tr>
                            ))
                        )}
                    </tbody>
            ...
            <div className="card" style={{ marginTop: 16 }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                    {escalation.map((row) => (
                        ...
                        >
                            {row.label} → <b style={{ color: '#C87D00' }}>{row.role}</b>
                        </span>
                    ))}
                </div>
            </div>
...
function Fact({ label, value }: { label: string; value: string }) {
    return (
        <div>
            <div style={{ fontSize: 11, color: 'rgba(10,10,10,.55)' }}>{label}</div>
            <div style={{ fontSize: 14, fontWeight: 700, color: '#0A0A0A' }}>{value}</div>
        </div>
    );
}
