...
export default function SupportConsole({ filters, results, escalation, pendingInvitations }: Props) {
    ...
    return (
        <AdminLayout>
            ...
            <PageHeader
                icon={Search}
                title={<>مركز الدعم — البحث والاستعلام الشامل</>}
                subtitle={<>
            ...
            <div className="card" style={{ marginBottom: 16 }}>
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                    <div style={{ position: 'relative', flex: 1, minWidth: 240 }}>
                        <Search className="w-4 h-4 text-[#0A0A0A]/40 absolute top-1/2 -translate-y-1/2 start-3.5 pointer-events-none" aria-hidden="true" />
                        <input
                            value={term}
                            onChange={(e) => setTerm(e.target.value)}
                            onKeyDown={(e) => e.key === 'Enter' && runSearch()}
                            placeholder="معرّف فعالية · اسم موظف · بريد · رقم جوال · اسم شركة"
                            style={{ ...inputStyle, width: '100%', paddingInlineStart: 38 }}
                        />
                    </div>
                    <button className="act-btn btn-approve" onClick={() => runSearch()}>بحث</button>
                </div>
                <div style={{ display: 'flex', gap: 6, marginTop: 10, flexWrap: 'wrap', alignItems: 'center' }}>
                    <span className="text-[11px] font-bold text-[#0A0A0A]/50 ms-1">النطاق:</span>
                    {SCOPES.map((s) => (
            ...
            {results.events.length > 0 && (
                <div className="card" style={{ padding: 0, overflowX: 'auto', marginBottom: 16 }}>
                    <div style={{ padding: '12px 16px', fontWeight: 700, color: '#0A0A0A' }}>الفعاليات</div>
                        ...
                        <tbody>
                            {results.events.map((event) => (
                                <tr key={event.id}>
                                    ...
                                    <td style={{ fontWeight: 700, color: '#0A0A0A' }}>{event.title}</td>
                                    <td style={{ color: '#0A0A0A' }}>{event.status}</td>
                                    <td style={{ fontSize: 12, color: 'rgba(10,10,10,.55)' }}>{event.event_date ?? '—'}</td>
            ...
            {results.employees.length > 0 && (
                <div className="card" style={{ padding: 0, overflowX: 'auto', marginBottom: 16 }}>
                    <div style={{ padding: '12px 16px', fontWeight: 700, color: '#0A0A0A' }}>الموظفون</div>
                    <table className="portal-table">
                        <thead>
                            <tr>
                                <th>الاسم</th>
                        ...
                        <tbody>
                            {results.employees.map((employee) => (
                                <tr key={employee.id}>
                                    <td style={{ fontWeight: 700, color: '#0A0A0A' }}>{employee.name}</td>
                                    <td dir="ltr" style={{ fontSize: 12, color: 'rgba(10,10,10,.55)' }}>{employee.email ?? '—'}</td>
                                    <td dir="ltr" style={{ fontSize: 12, color: 'rgba(10,10,10,.55)' }}>
                                        {employee.phone_tail ? `••••${employee.phone_tail}` : '—'}
                                    </td>
                                    <td style={{ fontSize: 12, color: '#0A0A0A' }}>{employee.company?.name ?? '—'}</td>
            ...
            {results.companies.length > 0 && (
                <div className="card" style={{ padding: 0, overflowX: 'auto', marginBottom: 16 }}>
                    <div style={{ padding: '12px 16px', fontWeight: 700, color: '#0A0A0A' }}>الشركات</div>
                    <table className="portal-table">
                        <thead>
                            <tr>
                                <th>الشركة</th>
                        ...
                        <tbody>
                            {results.companies.map((company) => (
                                <tr key={company.id}>
                                    ...
                                    <td dir="ltr" style={{ fontSize: 12, color: 'rgba(10,10,10,.55)' }}>{company.email ?? '—'}</td>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(320px,1fr))', gap: 16 }}>
                <div className="card">
                    ...
                    <p style={{ fontSize: 12, color: 'rgba(10,10,10,.55)', lineHeight: 1.9 }}>
                    ...
                    <form onSubmit={resendOtp}>
                        ...
                        {otpForm.errors.phone && <p style={{ fontSize: 12, color: '#D9381E' }}>{otpForm.errors.phone}</p>}
                ...
                <div className="card">
                    ...
                    <table className="portal-table">
                        ...
                        <tbody>
                            {escalation.map((row) => (
                                <tr key={row.action}>
                                    <td style={{ fontSize: 12, color: '#0A0A0A' }}>{row.label}</td>
                                    <td style={{ fontSize: 12, color: '#C87D00', fontWeight: 700 }}>{row.role}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
            ...
            <div className="card" style={{ padding: 0, overflowX: 'auto', marginTop: 16 }}>
                <div style={{ padding: '12px 16px', fontWeight: 700, color: '#0A0A0A' }}>دعوات معلّقة — إعادة الإرسال</div>
                <table className="portal-table">
                    <thead>
                        <tr>
                            <th>المدعو</th>
                        ...
                        {pendingInvitations.length === 0 ? (
                            ...
                            pendingInvitations.map((invitation) => (
                                <tr key={invitation.id}>
                                    <td>
                                        <div style={{ fontWeight: 700, color: '#0A0A0A' }}>{invitation.name ?? '—'}</div>
                                        <div dir="ltr" style={{ fontSize: 10, color: 'rgba(10,10,10,.55)' }}>
                                            {invitation.email ?? (invitation.phone_tail ? `••••${invitation.phone_tail}` : '—')}
                                        </div>
                                    </td>
                                    <td style={{ fontSize: 12, color: '#0A0A0A' }}>{invitation.company?.name ?? '—'}</td>
                                    <td>{invitation.send_count}</td>
                                    <td style={{ fontSize: 12, color: 'rgba(10,10,10,.55)' }}>{invitation.expires_at?.slice(0, 10) ?? '—'}</td>
                                    <td>
