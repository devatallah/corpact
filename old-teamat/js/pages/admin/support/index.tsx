...
const STATUS_META: Record<SupportMessage['status'], { label: string; color: string; bg: string }> = {
    new: { label: 'جديدة', color: '#C87D00', bg: 'rgba(224,176,64,0.12)' },
    in_progress: { label: 'قيد المعالجة', color: '#C87D00', bg: 'rgba(74,157,224,0.12)' },
    resolved: { label: 'تم الحل', color: '#2E7D32', bg: 'rgba(0,158,130,0.12)' },
};
...
export default function SupportIndex({ messages, stats, filters, sort }: Props) {
    ...
    return (
        <AdminLayout>
            ...
            <PageHeader
                icon={Headphones}
                title={<>رسائل الدعم — توثيق البلاغات والتصعيد</>}
                subtitle={<>
                ...
                <table className="portal-table">
                    ...
                    <tbody>
                        ...
                        {messages.data.map((message) => (
                            <tr key={message.id}>
                                <td>
                                    <div style={{ fontWeight: 700, color: '#0A0A0A' }}>{message.name}</div>
                                    <div style={{ fontSize: 12, color: 'rgba(10,10,10,.55)', direction: 'ltr', textAlign: 'right' }}>{message.email}</div>
                                </td>
                                <td style={{ color: '#0A0A0A', fontSize: 13 }}>{message.subject ?? '—'}</td>
                                <td style={{ color: '#0A0A0A', fontSize: 13, maxWidth: 280 }}>
                                ...
                                <td style={{ fontSize: 12, color: 'rgba(10,10,10,.55)' }}>{fmtDate(message.created_at)}</td>
                                <td>
                ...
                <div className="detail-overlay open" onClick={() => setDetail(null)}>
                    <div className="detail-panel" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div style={{ display: 'grid', gap: 10, marginTop: 16, fontSize: 13 }}>
                            <div><span style={{ color: 'rgba(10,10,10,.55)' }}>البريد: </span><span style={{ direction: 'ltr', unicodeBidi: 'embed', color: '#0A0A0A' }}>{detail.email}</span></div>
                            {detail.phone && <div><span style={{ color: 'rgba(10,10,10,.55)' }}>الجوال: </span><span style={{ direction: 'ltr', unicodeBidi: 'embed', color: '#0A0A0A' }}>{detail.phone}</span></div>}
                            {detail.subject && <div><span style={{ color: 'rgba(10,10,10,.55)' }}>الموضوع: </span><span style={{ color: '#0A0A0A' }}>{detail.subject}</span></div>}
                            <div><span style={{ color: 'rgba(10,10,10,.55)' }}>التاريخ: </span><span style={{ color: '#0A0A0A' }}>{fmtDate(detail.created_at)}</span></div>
                        </div>

                        <div style={{ marginTop: 16, padding: 14, background: '#FFFFFF', border: '0.5px solid rgba(10,10,10,.1)', borderRadius: 10, fontSize: 14, color: '#0A0A0A', lineHeight: 1.9, whiteSpace: 'pre-wrap' }}>
                            {detail.message}
                        </div>
