                    ...
                    <table className="portal-table">
                        ...
                        <tbody>
                            ...
                            {templates.data.map((template) => (
                                <tr key={template.id}>
                                    <td style={{ fontSize: 12, color: '#0A0A0A', whiteSpace: 'nowrap' }}>
                                        {GROUP_LABELS[template.group] ?? template.group}
                                    </td>
                                    <td style={{ direction: 'ltr', textAlign: 'right', fontSize: 12, color: 'rgba(10,10,10,.55)' }}>
                                        {template.key}
                                    </td>
                                    <td style={{ fontWeight: 700, color: '#0A0A0A', fontSize: 13 }}>{template.title_ar}</td>
                                    <td style={{ color: '#0A0A0A', fontSize: 13 }}>{template.audience ?? '—'}</td>
                                    <td>
                                        <ClassBadge value={template.class} />
                                    </td>
                                    <td style={{ fontSize: 12, color: 'rgba(10,10,10,.55)' }}>{(template.channels ?? []).join(' ← ')}</td>
                                    <td style={{ fontSize: 12, color: template.active ? '#2E7D32' : '#D9381E' }}>
            ...
            {editing && (
                <div className="detail-overlay open" onClick={() => setEditing(null)}>
                    <div className="detail-panel" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div style={{ direction: 'ltr', textAlign: 'right', fontSize: 12, color: 'rgba(10,10,10,.55)', marginTop: 4 }}>
                            {editing.key}
                        </div>

                        {(editing.variables ?? []).length > 0 && (
                            <div style={{ marginTop: 12, fontSize: 12, color: 'rgba(10,10,10,.55)' }}>
                                المتحوّلات المتاحة:{' '}
                        ...
                        <form onSubmit={submit} style={{ marginTop: 16 }}>
                            ...
                            {editing.class === 'mandatory' ? (
                                <div style={{ fontSize: 12, color: 'rgba(10,10,10,.55)', marginTop: 8 }}>
