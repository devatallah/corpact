...
const STATUS_COLORS: Record<string, string> = {
    queued: 'rgba(10,10,10,.55)',
    deferred: '#C87D00',
    sent: '#C87D00',
    delivered: '#2E7D32',
    failed: '#D9381E',
    skipped: 'rgba(10,10,10,.55)',
};
...
function StatusBadge({ status, label }: { status: string; label: string }) {
    const color = STATUS_COLORS[status] ?? 'rgba(10,10,10,.55)';
                ...
                <div style={{ overflowX: 'auto' }}>
                    <table className="portal-table">
                        ...
                        <tbody>
                            ...
                            {logs.data.map((log) => (
                                <tr key={log.id}>
                                    <td style={{ direction: 'ltr', textAlign: 'right', fontSize: 13, color: '#0A0A0A' }}>
                                        {log.recipient_phone ?? '—'}
                                    </td>
                                    <td style={{ direction: 'ltr', textAlign: 'right', fontSize: 12, color: 'rgba(10,10,10,.55)' }}>
                                        {log.template_key ?? '—'}
                                    </td>
                                    <td style={{ fontSize: 13, color: '#0A0A0A' }}>
                                        {CHANNEL_LABELS[log.channel] ?? log.channel}
                                    </td>
                                    <td style={{ fontSize: 13, color: '#0A0A0A' }}>{log.attempt}</td>
                                    <td>
                                        <StatusBadge status={log.status} label={statusLabel(log.status)} />
                                    </td>
                                    <td style={{ fontSize: 12, color: 'rgba(10,10,10,.55)' }}>{fmtDate(log.created_at)}</td>
                                    <td>
                ...
                <div className="detail-overlay open" onClick={() => setDetail(null)}>
                    <div className="detail-panel" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div style={{ display: 'grid', gap: 10, marginTop: 16, fontSize: 13 }}>
                            <div>
                                <span style={{ color: 'rgba(10,10,10,.55)' }}>القالب: </span>
                                <span style={{ direction: 'ltr', unicodeBidi: 'embed', color: '#0A0A0A' }}>
                                    {detail.template_key ?? '—'}
                                </span>
                            </div>
                            <div>
                                <span style={{ color: 'rgba(10,10,10,.55)' }}>المستلم: </span>
                                <span style={{ direction: 'ltr', unicodeBidi: 'embed', color: '#0A0A0A' }}>
                                    {detail.recipient_phone ?? '—'} {detail.recipient_type ? `(${detail.recipient_type}#${detail.recipient_id})` : ''}
                                </span>
                            </div>
                            <div>
                                <span style={{ color: 'rgba(10,10,10,.55)' }}>القناة والمحاولة: </span>
                            ...
                            {detail.reason && (
                                <div>
                                    <span style={{ color: 'rgba(10,10,10,.55)' }}>السبب: </span>
                                    <span style={{ color: '#0A0A0A' }}>{REASON_LABELS[detail.reason] ?? detail.reason}</span>
                                </div>
                            )}
                            {detail.provider_message_id && (
                                <div>
                                    <span style={{ color: 'rgba(10,10,10,.55)' }}>معرّف المزوّد: </span>
                                    <span style={{ direction: 'ltr', unicodeBidi: 'embed', color: '#0A0A0A' }}>
                                        {detail.provider_message_id}
                                    </span>
                                </div>
                        ...
                        {detail.recipient_phone && (
                            ...
                            >
                                ...
                                <div style={{ fontSize: 12, color: 'rgba(10,10,10,.55)', marginBottom: 8 }}>
                                ...
                                <div style={{ fontSize: 11, color: 'rgba(10,10,10,.55)', marginTop: 8, lineHeight: 1.8 }}>
                        ...
                        <div style={{ display: 'grid', gap: 6, marginTop: 16, fontSize: 12, color: 'rgba(10,10,10,.55)' }}>
                        ...
                        {detail.variables && Object.keys(detail.variables).length > 0 && (
                            ...
                            >
                                <div style={{ color: 'rgba(10,10,10,.55)', marginBottom: 6 }}>المتحوّلات</div>
                                {Object.entries(detail.variables).map(([key, value]) => (
                                    <div key={key}>
                                        <span style={{ direction: 'ltr', unicodeBidi: 'embed', color: 'rgba(10,10,10,.55)' }}>{key}</span>
                                        {': '}
                                        {String(value)}
                                    </div>
                                ))}
