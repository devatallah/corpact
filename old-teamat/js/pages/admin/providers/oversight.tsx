    ...
    return (
        <AdminLayout>
            ...
            <PageHeader
                ...
                subtitle="اعتماد الحسابات البنكية، تغييرات الأسعار، وتعديلات مؤشر الالتزام."
            />
            ...
            <div className="card" style={{ marginBottom: 16 }}>
                ...
                <div style={{ fontSize: 12, color: '#D9381E', marginBottom: 10 }}>
                    الاعتماد يدوي وشرط لأي صرف. أي تغيير بعد الاعتماد يعيد المزوّد إلى هذا الطابور ويُسجَّل حدثاً أمنياً.
                </div>
                {bankQueue.length === 0 && <div style={{ color: '#D9381E', fontSize: 13 }}>لا حسابات بانتظار الاعتماد.</div>}
                {bankQueue.map((p) => (
                    <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: '1px solid rgba(0,0,0,.08)', borderRadius: 10, padding: '10px 14px', marginBottom: 8, flexWrap: 'wrap', gap: 8 }}>
                        <div>
                            ...
                            {p.cr_number && <span style={{ fontSize: 12, color: '#D9381E', marginRight: 8 }}>س.ت {p.cr_number}</span>}
                            <div style={{ fontSize: 12, color: '#D9381E', marginTop: 2 }} dir="ltr">{p.bank_iban} · {p.bank_account_holder}</div>
                        </div>
                        <button className="act-btn" style={{ background: '#2E7D32', color: '#0A0A0A', borderColor: '#2E7D32', padding: '8px 18px', borderRadius: 8 }} onClick={() => setApproveBankFor(p)}>
                            اعتماد الحساب
                        </button>
                    </div>
                ))}
            ...
            <div className="card" style={{ marginBottom: 16 }}>
                ...
                {priceChanges.length === 0 && <div style={{ color: '#D9381E', fontSize: 13 }}>لا تعديلات معلّقة.</div>}
                {priceChanges.map((c) => (
                    <div key={c.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: '1px solid rgba(0,0,0,.08)', borderRadius: 10, padding: '10px 14px', marginBottom: 8, flexWrap: 'wrap', gap: 8 }}>
                        <div style={{ fontSize: 13 }}>
                            ...
                            <span style={{ margin: '0 6px', color: '#C87D00', textDecoration: 'line-through' }}>{Number(c.old_price).toLocaleString()}</span>
                            ← <b style={{ color: '#2E7D32' }}>{Number(c.new_price).toLocaleString()} ريال</b>
                        </div>
                        <div style={{ display: 'flex', gap: 6 }}>
                            <button className="act-btn" style={{ background: '#2E7D32', color: '#0A0A0A', borderColor: '#2E7D32', padding: '6px 14px', borderRadius: 8, fontSize: 12 }}
                                onClick={() => router.post(`/admin/providers/price-changes/${c.id}`, { decision: 'approved' }, { onSuccess: () => toastr.success('اعتُمد السعر') })}>
                                اعتماد
                            </button>
                            <button className="act-btn" style={{ color: '#C87D00', borderColor: '#C87D00', padding: '6px 14px', borderRadius: 8, fontSize: 12 }}
            ...
            <div className="card" style={{ marginBottom: 16 }}>
                ...
                <div className="frow">
                    ...
                    <div className="fg">
                        ...
                        {adjustForm.errors.delta && <div style={{ fontSize: 11, color: '#C87D00' }}>{adjustForm.errors.delta}</div>}
                    </div>
                </div>
                <div className="fg">
                    <label>السبب الموثَّق (إلزامي)</label>
                    <input value={adjustForm.data.reason} onChange={(e) => adjustForm.setData('reason', e.target.value)} />
                    {adjustForm.errors.reason && <div style={{ fontSize: 11, color: '#C87D00' }}>{adjustForm.errors.reason}</div>}
                </div>
                <button
                    className="act-btn"
                    style={{ background: '#0A0A0A', color: '#0A0A0A', borderColor: '#0A0A0A', padding: '9px 20px', borderRadius: 8, marginTop: 6 }}
            ...
            <div className="card">
                ...
                {recentAdjustments.length === 0 && <div style={{ color: '#D9381E', fontSize: 13 }}>لا تعديلات يدوية بعد.</div>}
                {recentAdjustments.map((log) => (
                    <div key={log.id} style={{ fontSize: 13, borderBottom: '1px solid rgba(0,0,0,.05)', padding: '8px 0' }}>
                        ...
                        <span style={{ color: log.delta > 0 ? '#2E7D32' : '#C87D00', margin: '0 6px' }}>({log.delta > 0 ? '+' : ''}{log.delta})</span>
                        <span style={{ color: '#D9381E' }}>— {log.note} · {fmtDateTime(log.created_at)}</span>
                    </div>
                ))}
            </div>
