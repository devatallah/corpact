...
function spikeColor(latest: number, baseline: number): string {
    if (baseline === 0) return latest > 0 ? '#C87D00' : '#2E7D32';
    if (latest >= baseline * 2) return '#D9381E';
    if (latest > baseline * 1.25) return '#C87D00';
    return '#2E7D32';
}
...
export default function GhostEventMonitor({ weeks, latest, baseline, companyId, companies, recentManualChanges, filters, sort }: Props) {
    ...
    return (
        <AdminLayout>
            ...
            <PageHeader
                title={<>الإنذار المبكر — الفعالية الشبح</>}
                subtitle={<>
            ...
            <div className="card" style={{ marginBottom: 16 }}>
                ...
                <select
                    ...
                    style={{ padding: '8px 10px', border: '0.5px solid rgba(10,10,10,.1)', borderRadius: 8, minWidth: 220 }}
                >
            ...
            <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
                <div style={{ padding: '14px 20px', borderBottom: '0.5px solid rgba(10,10,10,.1)' }}>
                    ...
                    <input
                        ...
                        style={{ marginTop: 10, width: '100%', maxWidth: 360, padding: '8px 12px', background: '#FFFFFF', border: '0.5px solid rgba(10,10,10,.1)', borderRadius: 10, fontSize: 12, color: '#0A0A0A', outline: 'none', direction: 'rtl', fontFamily: 'inherit' }}
                    />
                </div>
                <table className="portal-table">
                    <thead>
