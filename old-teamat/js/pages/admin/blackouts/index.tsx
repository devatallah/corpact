...
export default function BlackoutsIndex({ blackouts, totalBlackouts, filters, sort }: Props) {
    ...
    return (
        <AdminLayout>
            ...
            <div style={{ fontSize: 12, color: '#D9381E', marginBottom: 16 }}>
            ...
            <div className="card" style={{ marginBottom: 16 }}>
                ...
                <div className="frow">
                    <div className="fg">
                        ...
                        {form.errors.name && <div style={{ fontSize: 11, color: '#C87D00' }}>{form.errors.name}</div>}
                    </div>
                    <div className="fg">
                        <label>من تاريخ</label>
                        <input type="date" value={form.data.starts_on} onChange={(e) => form.setData('starts_on', e.target.value)} />
                        {form.errors.starts_on && <div style={{ fontSize: 11, color: '#C87D00' }}>{form.errors.starts_on}</div>}
                    </div>
                    <div className="fg">
                        <label>إلى تاريخ</label>
                        <input type="date" value={form.data.ends_on} onChange={(e) => form.setData('ends_on', e.target.value)} />
                        {form.errors.ends_on && <div style={{ fontSize: 11, color: '#C87D00' }}>{form.errors.ends_on}</div>}
                    </div>
                </div>
                <button
                    className="act-btn"
                    style={{ background: '#2E7D32', color: '#0A0A0A', borderColor: '#2E7D32', padding: '9px 20px', borderRadius: 8, marginTop: 6 }}
            ...
            <div className="card">
                ...
                {blackouts.data.length === 0 && (
                    <div style={{ color: '#D9381E', fontSize: 13 }}>
                ...
                {blackouts.data.map((b) => (
                    <div key={b.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: '1px solid rgba(0,0,0,.08)', borderRadius: 10, padding: '10px 14px', marginBottom: 8, flexWrap: 'wrap', gap: 8 }}>
                        <div>
                            <b>{b.name}</b>
                            <span style={{ fontSize: 12, color: '#D9381E', marginRight: 8 }}>
                                {fmtDate(b.starts_on)} — {fmtDate(b.ends_on)}
                            </span>
                        </div>
                        <button className="act-btn" style={{ color: '#C87D00', borderColor: '#C87D00', padding: '6px 14px', borderRadius: 8, fontSize: 12 }} onClick={() => setDeleting(b)}>
                            حذف
                        </button>
                    </div>
                ))}
