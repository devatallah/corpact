    ...
    return (
        <AdminLayout>
            ...
            <div style={{ marginBottom: 20 }}>
                ...
                <p style={{ fontSize: 13, color: 'rgba(10,10,10,.55)', marginTop: 6 }}>
            ...
            <div className="card" style={{ marginBottom: 20 }}>
                ...
                {failedRefunds.data.length === 0 ? (
                    <div style={{ fontSize: 13, color: 'rgba(10,10,10,.55)', textAlign: 'center', padding: 20 }}>
                    ...
                    <div style={{ overflowX: 'auto' }}>
                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                            <thead>
                                <tr style={{ textAlign: 'right', color: 'rgba(10,10,10,.55)' }}>
                            ...
                            <tbody>
                                {failedRefunds.data.map((row) => (
                                    <tr key={row.id} style={{ borderTop: '0.5px solid rgba(10,10,10,.1)' }}>
                                        <td style={{ padding: 8 }}>
                                            {row.event ? `#${row.event.id} ${row.event.title ?? ''}` : '—'}
                                            {row.community && <div style={{ fontSize: 11, color: 'rgba(10,10,10,.55)' }}>{row.community.name}</div>}
                                        </td>
                                        ...
                                        <td style={{ padding: 8 }}>
                                            ...
                                            {row.refund_attempts >= row.max_auto_retries && (
                                                <span style={{ display: 'inline-block', marginRight: 6, fontSize: 11, color: '#D9381E', fontWeight: 700 }}>يدوي</span>
                                            )}
                                        </td>
                                        <td style={{ padding: 8, maxWidth: 220, color: '#D9381E', fontSize: 12 }}>{row.refund_last_error ?? '—'}</td>
                                        <td style={{ padding: 8, fontSize: 12, color: 'rgba(10,10,10,.55)', whiteSpace: 'nowrap' }}>{fmtDateTime(row.updated_at)}</td>
                                        <td style={{ padding: 8 }}>
            ...
            <div className="card" style={{ marginBottom: 20 }}>
                ...
                {expiredIntents.length === 0 ? (
                    <div style={{ fontSize: 13, color: 'rgba(10,10,10,.55)', textAlign: 'center', padding: 20 }}>لا شيء.</div>
                ) : (
                    <div style={{ overflowX: 'auto' }}>
                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                            <thead>
                            ...
                            <tbody>
                                {failedWebhooks.map((row) => (
                                    <tr key={row.id} style={{ borderTop: '0.5px solid rgba(10,10,10,.1)' }}>
                                        ...
                                        <td style={{ padding: 8, color: '#D9381E', fontSize: 12, maxWidth: 260 }}>{row.error ?? '—'}</td>
