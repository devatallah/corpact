    ...
    return (
        <AdminLayout>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px' }}>
                <Link href="/admin/events" style={{ color: 'rgba(10,10,10,.55)', textDecoration: 'none', fontSize: '14px' }}>
                    ← الفعاليات
                </Link>
                <span style={{ color: '#0A0A0A' }}>/</span>
                <span style={{ color: '#0A0A0A', fontWeight: 700 }}>فعالية #{event.id}</span>
            </div>

            {/* Header */}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20, flexWrap: 'wrap', gap: 10 }}>
                <div>
                    <div style={{ fontSize: 20, fontWeight: 800, color: '#0A0A0A', marginBottom: 4 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 10, marginBottom: 20 }}>
                <div className="card">
                    <div style={{ fontSize: 11, color: 'rgba(10,10,10,.55)' }}>اللاعبون</div>
                    <div style={{ fontSize: 16, fontWeight: 800, color: fillPercent >= 100 ? '#2E7D32' : '#fff' }}>
                        {event.participants_count}/{event.capacity}
                    </div>
                    <div style={{ height: 4, background: 'rgba(10,10,10,.1)', borderRadius: 4, marginTop: 8, overflow: 'hidden' }}>
                ...
                <div className="card">
                    ...
                    <div style={{ fontSize: 16, fontWeight: 800, color: '#0A0A0A' }}>{event.venues_count}</div>
                </div>
                <div className="card">
                    <div style={{ fontSize: 11, color: 'rgba(10,10,10,.55)' }}>إجمالي التكلفة</div>
                    <div style={{ fontSize: 16, fontWeight: 800, color: '#C87D00' }}>{Number(event.total_amount).toLocaleString()} ريال</div>
                </div>
                <div className="card">
                    <div style={{ fontSize: 11, color: 'rgba(10,10,10,.55)' }}>حصة كل لاعب</div>
                    <div style={{ fontSize: 16, fontWeight: 800, color: '#0A0A0A' }}>{Number(event.cost_per_person).toLocaleString()} ريال</div>
                </div>
                {Number(event.community_contribution) > 0 && (
                    <div className="card">
                        <div style={{ fontSize: 11, color: 'rgba(10,10,10,.55)' }}>استقطاع من المحفظة</div>
                        <div style={{ fontSize: 16, fontWeight: 800, color: '#2E7D32' }}>{Number(event.community_contribution).toLocaleString()} ريال</div>
                    </div>
                )}
            </div>
            ...
            {event.template_id && (
                <div className="card" style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(10,10,10,.08)', borderColor: 'rgba(10,10,10,.27)' }}>
                    <span style={{ fontSize: 16 }}>🔄</span>
                    <span style={{ fontSize: 13, fontWeight: 700, color: '#0A0A0A' }}>مولّدة من قالب تكرار #{event.template_id}</span>
            ...
            {seriesEvents && seriesEvents.length > 0 && (
                <div className="card">
                    <div style={{ fontSize: 15, fontWeight: 800, color: '#0A0A0A', marginBottom: 14 }}>
                            ...
                            return (
                                <Link
                                    ...
                                    style={{
                                        ...
                                        borderRadius: 10,
                                        textDecoration: 'none',
                                        background: isCurrent ? '#2E7D3215' : '#FFFFFF',
                                        border: isCurrent ? '1px solid #2E7D3244' : '0.5px solid rgba(10,10,10,.1)',
                                        color: 'inherit',
                                    }}
                                >
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                        <span style={{ fontSize: 12, fontWeight: isCurrent ? 700 : 400, color: isCurrent ? '#2E7D32' : '#0A0A0A' }}>
                ...
                {joinedParticipants.length === 0 ? (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        {joinedParticipants.map((p) => (
                            <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', background: '#FFFFFF', borderRadius: 10 }}>
            ...
            {event.notes && (
                <div className="card">
                    <div style={{ fontSize: 13, fontWeight: 700, color: '#0A0A0A', marginBottom: 6 }}>ملاحظات</div>
                    <div style={{ fontSize: 13, color: '#0A0A0A' }}>{event.notes}</div>
                </div>
            )}
            ...
            <div className="card">
                ...
                {statusHistory.length === 0 ? (
                    <div style={{ textAlign: 'center', color: 'rgba(10,10,10,.55)', fontSize: 13, padding: '10px 0' }}>لا سجل بعد</div>
                ) : (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                        {statusHistory.map((h) => (
                            <div key={h.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', background: '#FFFFFF', borderRadius: 10, flexWrap: 'wrap' }}>
                                <span style={{ fontSize: 11, color: 'rgba(10,10,10,.55)', minWidth: 120 }}>{new Date(h.created_at).toLocaleString('ar-SA')}</span>
                                <span style={{ fontSize: 12, color: '#0A0A0A' }}>
                                    {h.from_status ? <StatusBadge status={h.from_status} /> : 'إنشاء'} ← <StatusBadge status={h.to_status} />
                                </span>
                                {h.is_manual && <span style={{ fontSize: 10, background: '#D9381E22', color: '#D9381E', padding: '2px 8px', borderRadius: 6, fontWeight: 700 }}>يدوي</span>}
                                {h.reason && <span style={{ fontSize: 11, color: 'rgba(10,10,10,.55)' }}>{h.reason}</span>}
                            </div>
                        ))}
                    </div>
                )}
            ...
            <div className="card" style={{ borderColor: '#D9381E44' }}>
                <div style={{ fontSize: 15, fontWeight: 800, color: '#D9381E', marginBottom: 6 }}>تغيير الحالة يدوياً</div>
                ...
                <form onSubmit={submitForceStatus} style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                    <select
                        ...
                        style={{ padding: '9px 12px', borderRadius: 10, background: '#FFFFFF', color: '#0A0A0A', border: '1px solid #0A0A0A', fontSize: 13, fontFamily: 'inherit' }}
                    >
                    ...
                    <input
                        ...
                        style={{ flex: 1, minWidth: 220, padding: '9px 12px', borderRadius: 10, background: '#FFFFFF', color: '#0A0A0A', border: '1px solid #0A0A0A', fontSize: 13, fontFamily: 'inherit', direction: 'rtl' }}
                    />
                    <button
                        type="submit"
            ...
            {attendance.length > 0 && (
                <div className="card" style={{ marginTop: 20 }}>
                    ...
                    <div style={{ fontSize: 12, color: 'rgba(10,10,10,.55)', lineHeight: 1.9, marginBottom: 14 }}>
                            ...
                            <tbody>
                                {attendance.map((row) => (
                                    <tr key={row.employee_id}>
                                        <td style={{ fontWeight: 700, color: '#0A0A0A' }}>{row.employee_name}</td>
                                        <td style={{ color: row.attendance_status === 'absent' ? '#D9381E' : '#2E7D32', fontWeight: 700 }}>
                                            {row.attendance_status === 'attended' ? 'حاضر' : row.attendance_status === 'absent' ? 'غائب' : '—'}
                                        </td>
                                        <td style={{ fontSize: 12, color: 'rgba(10,10,10,.55)' }}>{row.attendance_reason ?? '—'}</td>
                                        <td>
                                            <button
                    ...
                    {attendanceTarget && (
                        <div style={{ marginTop: 14, padding: 14, background: '#FFFFFF', border: '1px solid #0A0A0A', borderRadius: 10 }}>
                            ...
                            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                                <select
                                    ...
                                    style={{ padding: '9px 12px', borderRadius: 10, background: '#fff', color: '#0A0A0A', border: '0.5px solid rgba(10,10,10,.1)', fontSize: 13, fontFamily: 'inherit' }}
                                >
                                ...
                                <input
                                    ...
                                    style={{ flex: 1, minWidth: 220, padding: '9px 12px', borderRadius: 10, background: '#fff', color: '#0A0A0A', border: '0.5px solid rgba(10,10,10,.1)', fontSize: 13, fontFamily: 'inherit', direction: 'rtl' }}
                                />
                                <button
