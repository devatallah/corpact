...
export default function PermissionReview({ assignments, filters, roles, currentPeriod, lastReview, history, stats, sort }: Props) {
    ...
    return (
        <AdminLayout>
            ...
            <PageHeader
                title={<>مراجعة الصلاحيات الربع سنوية</>}
                subtitle={<>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(200px,1fr))', gap: 12, margin: '16px 0' }}>
                ...
                <StatCard
                    ...
                    color={stats.reviewed_this_period ? '#2E7D32' : '#C87D00'}
                />
            </div>
            ...
            <div className="card" style={{ marginBottom: 16 }}>
                ...
                {form.errors.notes && <p style={{ fontSize: 12, color: '#D9381E' }}>{form.errors.notes}</p>}
                    ...
                    <table className="portal-table">
                        ...
                        <tbody>
                            ...
                            {assignments.data.map((row) => (
                                <tr key={row.id}>
                                    <td>
                                        <div style={{ fontWeight: 700, color: '#0A0A0A' }}>{row.user?.name ?? '—'}</div>
                                        <div style={{ fontSize: 10, color: 'rgba(10,10,10,.55)' }} dir="ltr">{row.user?.email ?? '—'}</div>
                                    </td>
                                    <td style={{ color: '#0A0A0A', fontWeight: 700 }}>{row.role_label}</td>
                                    <td style={{ fontSize: 12, color: 'rgba(10,10,10,.55)' }}>{row.scope_label}</td>
                                    <td style={{ fontSize: 12, color: row.user?.status === 'active' ? '#2E7D32' : '#D9381E' }}>
                                        {row.user?.status === 'active' ? 'نشط' : 'غير نشط'}
                                    </td>
                                    <td style={{ fontSize: 12, color: 'rgba(10,10,10,.55)' }}>{row.granted_at ? fmtDateTime(row.granted_at) : '—'}</td>
                                    <td>
                                            ...
                                            <div style={{ marginTop: 8, display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                                                {row.permissions.map((permission) => (
                                                    <span
                                                        key={permission}
                                                        dir="ltr"
                                                        style={{ fontSize: 10, color: 'rgba(10,10,10,.65)', background: 'rgba(10,10,10,.05)', border: '0.5px solid rgba(10,10,10,.1)', borderRadius: 8, padding: '2px 6px' }}
                                                    >
                                                        {permission}
                                                    </span>
                                                ))}
                        ...
                        <tbody>
                            {history.map((review) => (
                                <tr key={review.period}>
                                    <td style={{ fontWeight: 700, color: '#0A0A0A' }}>{review.period}</td>
                                    <td style={{ fontSize: 12, color: 'rgba(10,10,10,.55)' }}>{review.reviewed_at ? fmtDateTime(review.reviewed_at) : '—'}</td>
                                    <td style={{ fontSize: 12, color: '#0A0A0A' }}>{review.reviewed_by ?? '—'}</td>
                                    <td>{review.assignments_reviewed}</td>
                                    <td style={{ fontSize: 12, color: 'rgba(10,10,10,.55)' }}>{review.notes ?? '—'}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
