...
const statusView: Record<string, { label: string; color: string; hint: string }> = {
    missing: { label: 'لم يُسجَّل حساب', color: 'rgba(10,10,10,.55)', hint: 'سجّل حسابك البنكي — لا صرف قبل تسجيله واعتماده.' },
    pending: { label: 'بانتظار اعتماد تيمات', color: '#C87D00', hint: 'الاعتماد يدوي من أدمن تيمات — الصرف محجوب حتى يكتمل.' },
    approved: { label: 'معتمد — الصرف متاح', color: '#2E7D32', hint: 'أي تغيير على الحساب يعيد الحالة إلى بانتظار الاعتماد ويحجب الصرف.' },
};
    ...
    return (
        <PartnerLayout>
            ...
            <div className="card" style={{ borderRight: `4px solid ${st.color}`, marginBottom: 16 }}>
                ...
                <div style={{ fontSize: 12, color: 'rgba(10,10,10,.55)', marginTop: 6 }}>{st.hint}</div>
                {bank.payouts_blocked && (
                    <div style={{ fontSize: 12, color: '#C87D00', fontWeight: 700, marginTop: 6 }}>
            ...
            <div className="card">
                <div className="frow">
                    <div className="fg">
                        ...
                        {form.errors.account_holder && <div style={{ fontSize: 11, color: '#C87D00', marginTop: 4 }}>{form.errors.account_holder}</div>}
                    </div>
                    <div className="fg">
                        ...
                        {(form.errors as Record<string, string>).iban && <div style={{ fontSize: 11, color: '#C87D00', marginTop: 4 }}>{(form.errors as Record<string, string>).iban}</div>}
                        {(form.errors as Record<string, string>).bank_iban && <div style={{ fontSize: 11, color: '#C87D00', marginTop: 4 }}>{(form.errors as Record<string, string>).bank_iban}</div>}
                    </div>
                </div>
                {canManage ? (
                    <button
                        className="act-btn"
                        style={{ background: '#0A0A0A', color: '#0A0A0A', borderColor: '#0A0A0A', padding: '10px 22px', borderRadius: 8 }}
                    ...
                    <div style={{ fontSize: 12, color: 'rgba(10,10,10,.55)' }}>ليس لديك صلاحية تعديل الحساب البنكي.</div>
                )}
            </div>
        </PartnerLayout>
    );
