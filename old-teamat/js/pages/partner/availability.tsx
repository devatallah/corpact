    ...
    return (
        <PartnerLayout>
            ...
            <div className="card" style={{ marginBottom: 16 }}>
                ...
                {(form.errors as Record<string, string>).slot && <div style={{ fontSize: 12, color: '#C87D00', marginBottom: 6 }}>{(form.errors as Record<string, string>).slot}</div>}
                <button
                    className="act-btn"
                    style={{ background: '#C87D00', color: '#0A0A0A', borderColor: '#C87D00', padding: '9px 20px', borderRadius: 8 }}
            ...
            <div className="card" style={{ marginBottom: 16 }}>
                ...
                {units.length === 0 ? (
                    <div style={{ textAlign: 'center', color: 'rgba(10,10,10,.55)', padding: 30 }}>أضف فروعاً ووحدات نشاط أولاً.</div>
                ) : (
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 8 }}>
                        {days.map((day, i) => (
                            <div key={day} style={{ border: '1px solid rgba(0,0,0,.08)', borderRadius: 10, padding: 8, minHeight: 110 }}>
                                <div style={{ fontSize: 12, fontWeight: 800, marginBottom: 6, textAlign: 'center' }}>{dayNames[i]}<br /><span style={{ color: 'rgba(10,10,10,.55)', fontWeight: 400 }}>{day.slice(5)}</span></div>
                                {selectedUnit !== '' && unitSlots(selectedUnit as number, day).map((slot) => (
                                    <div
                                        key={slot.id}
                                        onClick={() => slot.booking_type === 'external' && setDeleteSlot(slot)}
                                        style={{
                                            background: slot.booking_type === 'internal' ? '#2E7D3218' : '#C87D0018',
                                            color: slot.booking_type === 'internal' ? '#2E7D32' : '#C87D00',
                ...
                <div style={{ display: 'flex', gap: 14, marginTop: 10, fontSize: 11, color: 'rgba(10,10,10,.55)' }}>
                    <span><span style={{ display: 'inline-block', width: 10, height: 10, background: '#2E7D32', borderRadius: 3, marginLeft: 4 }} /> حجز منصة (لا يُحذف من هنا)</span>
                    <span><span style={{ display: 'inline-block', width: 10, height: 10, background: '#C87D00', borderRadius: 3, marginLeft: 4 }} /> حجز خارجي (اضغط للحذف)</span>
