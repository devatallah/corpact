...
function BranchForm({ initial, onSubmit, processing, errors }: {
    ...
    return (
        <div style={{ background: 'rgba(26,95,171,.05)', borderRadius: 10, padding: 14, marginTop: 10 }}>
            <div className="frow">
                <div className="fg"><label>اسم الفرع</label><input value={data.name} onChange={(e) => set('name', e.target.value)} />{errors.name && <div style={{ fontSize: 11, color: '#C87D00' }}>{errors.name}</div>}</div>
            ...
            <button className="act-btn" style={{ background: '#0A0A0A', color: '#0A0A0A', borderColor: '#0A0A0A', padding: '9px 20px', borderRadius: 8, marginTop: 6 }} disabled={processing || !data.name} onClick={() => onSubmit(data)}>
                حفظ الفرع
            </button>
        </div>
    );
    ...
    return (
        <div style={{ border: '1px solid rgba(0,0,0,.08)', borderRadius: 10, padding: '10px 14px', marginBottom: 8 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
                <div>
                    <b>{unit.name}</b>
                    <span style={{ fontSize: 12, color: 'rgba(10,10,10,.55)', marginRight: 8 }}>
                    ...
                    {unit.status !== 'active' && <span className="badge" style={{ background: 'rgba(10,10,10,.55)18', color: 'rgba(10,10,10,.55)', marginRight: 6 }}>{unit.status === 'maintenance' ? 'صيانة' : 'معطّلة'}</span>}
                    {pendingPriceChange && (
                        <span className="badge" style={{ background: '#C87D0018', color: '#C87D00', marginRight: 6 }}>
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                    ...
                    <button className="act-btn" style={{ fontSize: 12, padding: '6px 12px', borderRadius: 6, color: '#C87D00', borderColor: '#C87D00' }} onClick={() => setConfirmDelete(true)}>حذف</button>
                </div>
            </div>
            ...
            {editing && (
                <div style={{ marginTop: 10 }}>
                    ...
                    <div className="frow">
                        <div className="fg"><label>السعر (ريال، شامل الضريبة)</label><input type="number" value={form.data.price} onChange={(e) => form.setData('price', Number(e.target.value))} />{hasPriceContract && <div style={{ fontSize: 11, color: '#C87D00', marginTop: 3 }}>لديك عقد سعر — تعديل السعر يسري بعد اعتماد أدمن تيمات.</div>}</div>
                    ...
                    <button className="act-btn" style={{ background: '#2E7D32', color: '#0A0A0A', borderColor: '#2E7D32', padding: '8px 18px', borderRadius: 8 }} disabled={form.processing}
...
function AddUnitForm({ branchId, categories }: { branchId: number; categories: Category[] }) {
    ...
    return (
        <div style={{ background: 'rgba(26,122,74,.05)', borderRadius: 10, padding: 12, marginTop: 8 }}>
            <div className="frow">
                <div className="fg"><label>الاسم (ملعب/مسار/قاعة)</label><input value={form.data.name} onChange={(e) => form.setData('name', e.target.value)} />{form.errors.name && <div style={{ fontSize: 11, color: '#C87D00' }}>{form.errors.name}</div>}</div>
                <div className="fg">
                    <label>النشاط</label>
                    <select value={form.data.category_id} onChange={(e) => form.setData('category_id', e.target.value)}>
                        {categories.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
            ...
            <div className="frow">
                <div className="fg"><label>السعر</label><input type="number" value={form.data.price} onChange={(e) => form.setData('price', Number(e.target.value))} />{form.errors.price && <div style={{ fontSize: 11, color: '#C87D00' }}>{form.errors.price}</div>}</div>
                <div className="fg"><label>المدة الافتراضية (د)</label><input type="number" value={form.data.default_duration_minutes} onChange={(e) => form.setData('default_duration_minutes', Number(e.target.value))} /></div>
                <div className="fg"><label>الطاقة الدنيا</label><input type="number" value={form.data.min_capacity} onChange={(e) => form.setData('min_capacity', Number(e.target.value))} /></div>
                <div className="fg"><label>الطاقة القصوى</label><input type="number" value={form.data.max_capacity} onChange={(e) => form.setData('max_capacity', Number(e.target.value))} /></div>
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
                <button className="act-btn" style={{ background: '#2E7D32', color: '#0A0A0A', borderColor: '#2E7D32', padding: '8px 18px', borderRadius: 8 }} disabled={form.processing || !form.data.name}
    ...
    return (
        <PartnerLayout>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                ...
                <button className="act-btn" style={{ background: '#0A0A0A', color: '#0A0A0A', borderColor: '#0A0A0A', padding: '9px 18px', borderRadius: 8 }} onClick={() => setShowCreate(!showCreate)}>
                    + فرع جديد
                </button>
            </div>

            <div className="card" style={{ marginBottom: 16, fontSize: 12, color: 'rgba(10,10,10,.55)' }}>
            ...
            {items.length === 0 && (
                <div className="card" style={{ textAlign: 'center', color: 'rgba(10,10,10,.55)', padding: 40 }}>
            ...
            {items.map((branch) => (
                <div key={branch.id} className="card" style={{ marginBottom: 16 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                        <div>
                            <div style={{ fontSize: 16, fontWeight: 800 }}>{branch.name} {branch.status !== 'active' && <span className="badge" style={{ background: 'rgba(10,10,10,.55)18', color: 'rgba(10,10,10,.55)' }}>غير نشط</span>}</div>
