    ...
    return (
        <PartnerLayout>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
                <Link href="/partner/venues" style={{ color: 'rgba(10,10,10,.55)', textDecoration: 'none', fontSize: 14 }}>← المرافق</Link>
                <span style={{ color: '#0A0A0A' }}>/</span>
                <span style={{ fontWeight: 700 }}>تعديل: {venue.name}</span>
            </div>

            {/* Errors */}
            {Object.keys(form.errors).length > 0 && (
                <div style={{ background: '#C87D0010', border: '1px solid #C87D0033', borderRadius: 10, padding: '12px 16px', marginBottom: 20 }}>
                    {Object.values(form.errors).map((error, i) => (
                        <p key={i} style={{ fontSize: 12, color: '#C87D00', margin: '0 0 4px' }}>{error}</p>
                    ))}
                </div>
            )}

            <div style={{ background: '#fff', border: '0.5px solid rgba(10,10,10,.1)', borderRadius: 16, padding: 32, maxWidth: 500 }}>
                <div style={{ fontSize: 18, fontWeight: 700, marginBottom: 6 }}>تعديل المرفق</div>
                <div style={{ fontSize: 13, color: 'rgba(10,10,10,.55)', marginBottom: 20 }}>
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div style={{ display: 'flex', gap: 10 }}>
                        <button
                            ...
                            style={{ flex: 1, background: '#C87D00', color: '#0A0A0A', borderColor: '#C87D00' }}
                        >
                            حفظ التعديلات
                        </button>
                        <Link
