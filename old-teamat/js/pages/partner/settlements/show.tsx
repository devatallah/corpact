    ...
    return (
        <PartnerLayout>
            ...
            <div style={{ marginBottom: 16 }}>
                <Link href="/partner/settlements" style={{ color: '#0A0A0A', fontWeight: 700 }}>
                    ← كل الكشوف
                </Link>
            </div>

            <div style={{ marginBottom: 20 }}>
                <PageHeader title={<>كشف الفترة {statement.period_key} <StatusBadge status={statement.status} /></>} subtitle={<>{statement.period_start} → {statement.period_end} · {statement.items_count} بند</>} />
            </div>
                ...
                <table className="portal-table">
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td style={{ color: '#C87D00', fontWeight: 700 }}>−{statement.commission_amount} ر</td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td style={{ fontWeight: 800, color: '#2E7D32' }}>{statement.net_amount} ر</td>
                        </tr>
                        {statement.paid_at && (
                            <>
                                <tr>
                            ...
                            {items.data.map((item) => (
                                <tr key={item.id}>
                                    <td style={{ fontWeight: 700 }}>
                                        ...
                                        {item.reason && (
                                            <div style={{ fontSize: 11, color: '#C87D00' }}>
                                    ...
                                    <td style={{ color: '#C87D00' }}>{item.commission_amount} ر</td>
                                    <td style={{ fontWeight: 700 }}>{item.net_amount} ر</td>
                                    <td>
                                        {item.type === 'correction' ? (
                                            <span style={{ color: '#C87D00', fontWeight: 700 }}>بند تصحيحي</span>
                                        ) : (
                                            'فعالية'
                                        )}
                                    </td>
