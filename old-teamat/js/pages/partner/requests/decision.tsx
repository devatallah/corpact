...
const statusLabels: Record<string, { label: string; color: string }> = {
    pending: { label: 'بانتظار قرارك', color: '#C87D00' },
...
export default function ProviderRequestDecision({ request, can_decide }: Props) {
    ...
    return (
        <PartnerLayout>
            ...
            {/* القاعدة الذهبية */}
            <div className="card" style={{ borderRight: '4px solid #C87D00', marginBottom: 16, fontSize: 13, color: 'rgba(10,10,10,.55)' }}>
            ...
            <div className="card" style={{ marginBottom: 16 }}>
                ...
                <div className="req-grid">
                    ...
                    <div className="ri"><div className="rl">💰 الإجمالي (كمية نهائية)</div><div className="rv" style={{ color: '#2E7D32', fontSize: 16 }}>{Number(request.total_amount ?? 0).toLocaleString()} ريال</div></div>
                    <div className="ri"><div className="rl">⏳ مهلة الرد</div><div className="rv" style={{ color: deadlinePassed ? '#C87D00' : '#C87D00' }}>{request.deadline_at ? fmtDateTime(request.deadline_at) : '—'}{deadlinePassed && request.status === 'pending' ? ' (تجاوزت المهلة — الرد الآن متأخر)' : ''}</div></div>
                </div>
                ...
                <div style={{ background: 'rgba(26,95,171,.06)', borderRadius: 10, padding: '10px 14px', marginTop: 12, fontSize: 13 }}>
                    ...
                    <div style={{ fontSize: 11, color: 'rgba(10,10,10,.55)', marginTop: 4 }}>لا تظهر لك أسماء المشاركين ولا أرقامهم — عددهم فقط.</div>
                </div>

                {request.rejection_reason && (
                    <div style={{ marginTop: 10, fontSize: 13, color: '#C87D00' }}>سبب الرفض: {request.rejection_reason}</div>
                )}
                {request.cancellation_reason && (
                    <div style={{ marginTop: 10, fontSize: 13, color: '#C87D00' }}>سبب الإلغاء: {request.cancellation_reason}</div>
                )}
            </div>
            ...
            {can_decide && (
                <div className="card" style={{ marginBottom: 16 }}>
                    <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                        <button className="act-btn" style={{ flex: 1, background: '#2E7D32', color: '#0A0A0A', borderColor: '#2E7D32', padding: '12px', borderRadius: 10, fontWeight: 800 }} onClick={accept}>
                            ✓ قبول — يحجز الوحدة فوراً
                        </button>
                        <button className="act-btn" style={{ flex: 1, background: '#C87D0018', color: '#C87D00', borderColor: '#C87D00', padding: '12px', borderRadius: 10, fontWeight: 800 }} onClick={() => setMode(mode === 'reject' ? 'none' : 'reject')}>
                            ✕ رفض
                        </button>
                        <button className="act-btn" style={{ flex: 1, background: '#0A0A0A18', color: '#0A0A0A', borderColor: '#0A0A0A', padding: '12px', borderRadius: 10, fontWeight: 800 }} onClick={() => setMode(mode === 'alternative' ? 'none' : 'alternative')}>
                            ⏱ اقتراح وقت بديل
                        </button>
                    </div>
                    ...
                    {mode === 'reject' && (
                        <div style={{ marginTop: 14 }}>
                            ...
                            {rejectForm.errors.reason && <div style={{ fontSize: 11, color: '#C87D00', marginTop: 4 }}>{rejectForm.errors.reason}</div>}
                            <button
                                className="act-btn"
                                style={{ marginTop: 10, background: '#C87D00', color: '#0A0A0A', borderColor: '#C87D00', padding: '10px 20px', borderRadius: 8 }}
                        ...
                        <div style={{ marginTop: 14 }}>
                            <div className="frow">
                                <div className="fg">
                                    ...
                                    {altForm.errors.proposed_date && <div style={{ fontSize: 11, color: '#C87D00', marginTop: 4 }}>{altForm.errors.proposed_date}</div>}
                                </div>
                                <div className="fg">
                                    <label>وقت البداية</label>
                                    <TimePicker value={altForm.data.proposed_start_time} onChange={(v) => altForm.setData('proposed_start_time', v)} />
                                    {altForm.errors.proposed_start_time && <div style={{ fontSize: 11, color: '#C87D00', marginTop: 4 }}>{altForm.errors.proposed_start_time}</div>}
                                </div>
                            </div>
                            ...
                            <button
                                className="act-btn"
                                style={{ marginTop: 10, background: '#0A0A0A', color: '#0A0A0A', borderColor: '#0A0A0A', padding: '10px 20px', borderRadius: 8 }}
            ...
            {request.status === 'accepted' && (
                <div className="card" style={{ borderRight: '4px solid #C87D00' }}>
                    <div style={{ fontSize: 14, fontWeight: 800, color: '#C87D00', marginBottom: 8 }}>إلغاء الحجز بعد القبول</div>
                    ...
                    {mode !== 'cancel' ? (
                        <button className="act-btn" style={{ background: '#C87D0018', color: '#C87D00', borderColor: '#C87D00', padding: '10px 18px', borderRadius: 8 }} onClick={() => setMode('cancel')}>
                            أريد الإلغاء
                        </button>
                    ) : (
                        <div>
                            ...
                            {cancelForm.errors.reason && <div style={{ fontSize: 11, color: '#C87D00', marginTop: 4 }}>{cancelForm.errors.reason}</div>}
