...
function Stat({ label, value, hint }: { label: string; value: string; hint?: string }) {
    return (
        <div className="card" style={{ textAlign: 'center', flex: 1, minWidth: 160 }}>
            <div style={{ fontSize: 26, fontWeight: 800, color: '#0A0A0A' }}>{value}</div>
            <div style={{ fontSize: 13, fontWeight: 700, marginTop: 4 }}>{label}</div>
            {hint && <div style={{ fontSize: 11, color: 'rgba(10,10,10,.55)', marginTop: 4 }}>{hint}</div>}
        </div>
    );
}
...
export default function Reliability({ behaviors }: Props) {
    ...
    return (
        <PartnerLayout>
            ...
            <div className="card" style={{ fontSize: 13, color: 'rgba(10,10,10,.55)', lineHeight: 2 }}>
                ...
                <div style={{ fontSize: 11, color: 'rgba(10,10,10,.55)', marginTop: 6 }}>
