...
return [];
    ...
    useEffect(() => {
        ...
        fetch('/employee/create/pricings', {
            ...
            .catch(() => {
 setServerPricings([]); setLoadingPricings(false); 
});
    ...
    const availablePricings = useMemo(() => {
        if (selectedVenues.length === 0 || serverPricings.length === 0) {
return [];
}
    ...
    const venuePrices = useMemo(() => {
        if (!selectedPricing || selectedVenues.length === 0) {
return [];
}
    ...
    return (
        <EmployeeLayout>
            ...
            <div style={{ marginBottom: 28 }}>
                <h1 className="text-lg font-black text-[#0A0A0A]">إنشاء فعالية</h1>
                <p className="text-[11px] text-[#0A0A0A]/55 mt-0.5">اختر تفاصيل الفعالية الجديدة</p>
            </div>
            ...
            <form onSubmit={handleSubmit}>
                            ...
                            communities.map((community) => {
                                ...
                                return (
                                    <div
                                        ...
                                        className="card"
                                        style={{ display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer', borderColor: isSelected ? '#0A0A0A' : undefined }}
                                    >
                                        <div><CategoryIcon icon={community.category?.icon} size={28} /></div>
                                        <div style={{ flex: 1 }}>
                                            <div style={{ fontSize: 14, fontWeight: 600 }}>{community.name}</div>
                                            <div style={{ fontSize: 12, color: 'rgba(10,10,10,.5)', marginTop: 2 }}>{community.members_count} عضو</div>
                                        </div>
                                        {isSelected && <div style={{ color: '#0A0A0A', fontWeight: 700, fontSize: 18 }}>✓</div>}
                                    </div>
                                );
                            })
                        ) : (
                ...
                {step === 2 && (
                    <div>
                        ...
                        <div style={{ marginBottom: 16 }}>
                            <label style={{ fontSize: 13, color: 'rgba(10,10,10,.6)', marginBottom: 6, display: 'block' }}>اختر الشريك</label>
                            <select
                        ...
                        {selectedPartner && availablevenues.length > 0 && (
                            <div style={{ marginBottom: 16 }}>
                                <label style={{ fontSize: 13, color: 'rgba(10,10,10,.6)', marginBottom: 6, display: 'block' }}>اختر المرافق ({data.venue_ids.length})</label>
                                        ...
                                        return (
                                            <div
                                                ...
                                                style={{
                                                    ...
                                                    cursor: 'pointer',
                                                    marginBottom: 0,
                                                    borderColor: isSelected ? '#0A0A0A' : undefined,
                                                    background: isSelected ? '#0A0A0A08' : '#fff',
                                                }}
                                            >
                                                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                                    <div style={{ width: 18, height: 18, borderRadius: 4, ...(isSelected ? { background: '#0A0A0A', display: 'flex', alignItems: 'center', justifyContent: 'center' } : { border: '0.5px solid rgba(10,10,10,.1)' }) }}>
                                                        {isSelected && <span style={{ color: '#0A0A0A', fontSize: 12, fontWeight: 700 }}>✓</span>}
                                                    </div>
                                                    <span style={{ fontSize: 14, fontWeight: 600 }}>{venue.name}</span>
                                                </div>
                                                <span style={{ fontSize: 12, color: 'rgba(10,10,10,.5)' }}>{venue.category?.name ?? ''}</span>
                                            </div>
                                        );
                                    })}
                                </div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
                            <div>
                                <label style={{ fontSize: 13, color: 'rgba(10,10,10,.6)', marginBottom: 6, display: 'block' }}>التاريخ</label>
                                <input
                                    type="date"
                                    value={data.date}
                            ...
                            <div>
                                <label style={{ fontSize: 13, color: 'rgba(10,10,10,.6)', marginBottom: 6, display: 'block' }}>الوقت</label>
                        ...
                        <div style={{ marginBottom: 16 }}>
                            <label style={{ fontSize: 13, color: 'rgba(10,10,10,.6)', fontWeight: 600, marginBottom: 8, display: 'block' }}>مدة الفعالية</label>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                {loadingPricings ? (
                                    <div className="card" style={{ textAlign: 'center', color: 'rgba(10,10,10,.5)', fontSize: 13, marginBottom: 0 }}>جاري تحميل الأسعار...</div>
                                        ...
                                        return (
                                            ...
                                            >
                                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                                        <div style={{ width: 18, height: 18, borderRadius: '50%', ...(isSelected ? { background: '#0A0A0A', display: 'flex', alignItems: 'center', justifyContent: 'center' } : { border: '0.5px solid rgba(10,10,10,.1)' }) }}>
                                                        ...
                                                        {p.label && <span style={{ fontSize: 11, color: 'rgba(10,10,10,.5)' }}>{p.label}</span>}
                                                    </div>
                                                    <span style={{ fontSize: 14, fontWeight: 700, color: isSelected ? '#0A0A0A' : '#0A0A0A' }}>{durationTotal.toLocaleString()} ريال</span>
                                                </div>
                                                {selectedVenues.length > 1 && (
                                                    <div style={{ marginTop: 8, marginRight: 26, display: 'flex', flexDirection: 'column', gap: 2 }}>
                                                        {perVenue.map((c) => (
                                                            <div key={c.name} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: 'rgba(10,10,10,.5)' }}>
                                                                <span>{c.name}</span>
                                    ...
                                    <div className="card" style={{ textAlign: 'center', color: 'rgba(10,10,10,.5)', fontSize: 13, marginBottom: 0 }}>
                        ...
                        {/* Players count */}
                        <div style={{ marginBottom: 16 }}>
                            <label style={{ fontSize: 13, color: 'rgba(10,10,10,.6)', fontWeight: 600, marginBottom: 8, display: 'block' }}>عدد اللاعبين</label>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                ...
                                <input
                                    ...
                                    style={{ flex: 1, textAlign: 'center', fontSize: 18, fontWeight: 700, borderColor: '#0A0A0A' }}
                                />
                        ...
                        <div style={{ marginBottom: 16 }}>
                            <label style={{ fontSize: 13, color: 'rgba(10,10,10,.6)', fontWeight: 600, marginBottom: 8, display: 'block' }}>الحد الأدنى للمشاركين</label>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        ...
                        {/* Live price summary */}
                        <div className="card" style={{ background: '#E8F5E9', borderColor: '#0A0A0A33', marginBottom: 20 }}>
                            {/* Wallet balance */}
                            ...
                            {venuePrices.map((c) => (
                                <div key={c.name} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3, paddingRight: 10 }}>
                                    <span style={{ fontSize: 12, color: 'rgba(10,10,10,.5)' }}>{c.name}</span>
                                    <span style={{ fontSize: 12, color: 'rgba(10,10,10,.5)' }}>{c.price.toLocaleString()} ريال</span>
                                </div>
                            ))}
                            ...
                            {communityContribution > 0 && (
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 8, marginBottom: 4 }}>
                                    <span style={{ fontSize: 13, color: 'rgba(10,10,10,.6)' }}>استقطاع من المحفظة</span>
                                    <span style={{ fontSize: 13, fontWeight: 600, color: '#0A0A0A' }}>{communityContribution.toLocaleString()} ريال</span>
                                </div>
                            )}
                            ...
                            {communityContribution > 0 && (
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                                    <span style={{ fontSize: 13, color: 'rgba(10,10,10,.6)' }}>المتبقي على اللاعبين</span>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                ...
                                <span style={{ fontSize: 22, fontWeight: 700, color: '#0A0A0A' }}>{maxShare.toLocaleString()} ريال</span>
                            </div>
                            {maxShare > 0 && (
                                <div style={{ marginTop: 4, fontSize: 12, color: 'rgba(10,10,10,.6)', textAlign: 'left' }}>
                ...
                {step === 3 && (
                    <div>
                        ...
                        <div className="card" style={{ borderColor: '#0A0A0A33', marginBottom: 20 }}>
                            {reviewRows.map((row, i) => (
                                <div key={row.label} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', ...(i < reviewRows.length - 1 ? { borderBottom: '0.5px solid rgba(10,10,10,.1)' } : {}) }}>
                                    <span style={{ fontSize: 13, color: 'rgba(10,10,10,.5)' }}>{row.label}</span>
                                    <span style={{ fontSize: 13, fontWeight: 600 }}>{row.value}</span>
                                </div>
                            ))}
                            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0' }}>
                                <span style={{ fontSize: 13, color: 'rgba(10,10,10,.5)' }}>حصة الفرد بحد أقصى</span>
                                <span style={{ fontSize: 16, fontWeight: 700, color: '#0A0A0A' }}>{maxShare.toLocaleString()} ريال</span>
                            </div>
                        </div>

                        <div className="card" style={{ background: '#E8F5E9', borderColor: '#0A0A0A33', fontSize: 13, color: '#0A0A0A', marginBottom: 20 }}>
                            سيُرسل طلب الفعالية للشريك بعد اكتمال عدد اللاعبين
                        </div>

                        {Object.keys(errors).length > 0 && (
                            <div className="card" style={{ background: '#FDEDEC', borderColor: 'rgba(217,56,30,.25)', marginBottom: 16 }}>
