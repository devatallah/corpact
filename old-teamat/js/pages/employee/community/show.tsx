...
return `منذ ${days} يوم`;
    ...
    return (
        <EmployeeLayout>
            ...
            <div className="card" style={{ marginTop: 8 }}>
                ...
                <div style={{ display: 'flex', gap: 24, alignItems: 'center' }}>
                    <div style={{ textAlign: 'center' }}>
                        ...
                        <div style={{ fontSize: 12, color: 'rgba(10,10,10,.5)' }}>عضو</div>
                    </div>
                    <div style={{ textAlign: 'center' }}>
                        <div style={{ fontSize: 22, fontWeight: 700, color }}>{community.events_count ?? 0}</div>
                        <div style={{ fontSize: 12, color: 'rgba(10,10,10,.5)' }}>فعالية نشطة</div>
                    </div>
                    <div style={{ flex: 1 }} />
            ...
            {/* Balance */}
            <div className="card">
                <div style={{ fontSize: 12, color: 'rgba(10,10,10,.5)', marginBottom: 4, fontWeight: 500 }}>رصيد المجتمع</div>
                ...
                <div>
                    {isLeader && (
                        <Link
                            ...
                            style={{
                                ...
                                border: '2px dashed rgba(10,10,10,.15)', color, fontWeight: 600, fontSize: 14,
                            }}
                        >
                            <span>🔄 قوالب التكرار — محرك التشغيل التلقائي</span>
                            <span style={{ fontSize: 12, color: 'rgba(10,10,10,.5)' }}>إدارة ←</span>
                        </Link>
                    )}
                    ...
                    {isLeader && (
                        ...
                        >
                            ...
                            <span style={{ fontSize: 11, color: 'rgba(10,10,10,.55)' }}>بلا بيانات مالية ولا أرقام جوال</span>
                            <span style={{ flex: 1 }} />
                            {[
                                { key: 'events_results', label: 'الفعاليات والنتائج' },
                                { key: 'employees_activation', label: 'الأعضاء والتفعيل' },
                            ].map((item) => (
                                <span key={item.key} style={{ display: 'inline-flex', border: '0.5px solid rgba(10,10,10,.15)', borderRadius: 8, overflow: 'hidden' }}>
                                    <span style={{ padding: '6px 9px', fontSize: 12, background: '#F6F8F5' }}>{item.label}</span>
                                    <a
                                        href={`/employee/community/${community.id}/exports/${item.key}?format=xlsx`}
                                        style={{ padding: '6px 9px', fontSize: 12, borderRight: '0.5px solid rgba(10,10,10,.15)', color: '#0A0A0A' }}
                                    >
                                        Excel
                                    </a>
                                    <a
                        ...
                        eventItems.map((event) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div style={{ fontSize: 13, color: 'rgba(10,10,10,.5)', margin: '4px 0 10px' }}>
                                        {fmtDate(event.event_date)} · {fmtTime(event.start_time)}
                                    </div>
                                    <div className="bar-wrap">
                                        <div className="bar-fill" style={{ width: `${pct}%`, background: color }} />
                                    </div>
                                    <div style={{ fontSize: 12, color: 'rgba(10,10,10,.5)', marginTop: 6 }}>
                            ...
                            <div key={a.id} className="card">
                                {editingAnnouncementId === a.id ? (
                                    ...
                                    <>
                                        ...
                                        <div style={{ fontSize: 12, color: 'rgba(10,10,10,.5)', marginTop: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                            <span>
            ...
            {activeTab === 'polls' && (
                <div>
                    {canAnnounce && (
                        <form onSubmit={submitPoll} className="card" style={{ border: '2px dashed rgba(10,10,10,.15)', marginBottom: 20 }}>
                            ...
                            />
                            <div style={{ fontSize: 12, fontWeight: 600, color: 'rgba(10,10,10,.5)', marginBottom: 8 }}>الخيارات</div>
                            ...
                            <div style={{ marginTop: 8, marginBottom: 12 }}>
                                <label style={{ fontSize: 12, color: 'rgba(10,10,10,.5)', fontWeight: 500, display: 'block', marginBottom: 6 }}>
                    ...
                    {polls.length > 0 ? (
                        polls.map((poll) => {
                            ...
                            return (
                                <div key={poll.id} className="card" style={{ borderRight: `3px solid ${expired ? 'rgba(10,10,10,.5)' : color}` }}>
                                    {/* Header */}
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                                        <div style={{ flex: 1, minWidth: 0 }}>
                                            <div style={{ fontSize: 15, fontWeight: 600, lineHeight: 1.5 }}>{poll.question}</div>
                                            <div style={{ fontSize: 12, color: 'rgba(10,10,10,.5)', marginTop: 4 }}>
                                                ...
                                                return (
                                                    <div
                                                        ...
                                                        style={{
                                                            ...
                                                            border: isMyVote ? `2px solid ${color}` : '0.5px solid rgba(10,10,10,.1)',
                                                            background: '#fff',
                                                        }}
                                                    >
                                                        <div
                                                            style={{
                                                                ...
                                                                width: `${pct}%`, background: isMyVote ? `${color}18` : '#F6F8F5',
                                                        ...
                                                        <div style={{
                                                            ...
                                                            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                                                <span style={{ fontSize: 12, fontWeight: 600, color: 'rgba(10,10,10,.5)' }}>
                                                                    {option.votes_count}
                                                                </span>
                                                                <span style={{ fontSize: 12, fontWeight: 600, color: isMyVote ? color : 'rgba(10,10,10,.5)' }}>
                                                                    {pct}%
                                                                </span>
                                                            </div>
                                                        </div>
                                    ...
                                    {/* Footer */}
                                    <div style={{ fontSize: 12, color: 'rgba(10,10,10,.5)', marginTop: 10, display: 'flex', justifyContent: 'space-between' }}>
                                ...
                                return (
                                    ...
                                    >
                                        <div style={{ flex: 1, minWidth: 0 }}>
                                            ...
                                            <div style={{ fontSize: 12, color: 'rgba(10,10,10,.5)', marginTop: 3 }}>
            ...
            {activeTab === 'members' && (
                <div>
                    {canInvite && invitableEmployees.length > 0 && (
                        <form onSubmit={inviteEmployee} className="card" style={{ display: 'flex', gap: 8, alignItems: 'center', border: '2px dashed rgba(10,10,10,.15)' }}>
                            ...
                            {members.map((member) => (
                                <div key={member.id} className="list-row" style={{ cursor: 'default' }}>
                                    ...
                                    <div style={{ flex: 1 }}>
                                        ...
                                        <div style={{ fontSize: 12, color: 'rgba(10,10,10,.5)' }}>{member.department?.name ?? '—'}</div>
                                    </div>
