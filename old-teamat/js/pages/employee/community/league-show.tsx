...
export default function LeagueShow({ community, league, standings, isLeader }: Props) {
    const color = community.category?.color ?? community.color ?? '#0A0A0A';
    ...
    return (
        <EmployeeLayout>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8, paddingTop: 4 }}>
                <Link href={`/employee/community/${community.id}?tab=leagues`} style={{ color: 'rgba(10,10,10,.5)', textDecoration: 'none', fontSize: 13 }}>← {community.name}</Link>
            ...
            {activeView === 'standings' && standings && (
                <div className="card" style={{ padding: 0, overflow: 'auto' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12, whiteSpace: 'nowrap' }}>
                        ...
                        <tbody>
                            {standings.map((row, i) => (
                                <tr key={row.department.id} style={{ borderTop: '0.5px solid rgba(10,10,10,.1)', background: i === 0 ? `${color}06` : undefined }}>
                                    <td style={{ padding: '10px 8px', textAlign: 'center', fontWeight: 600, color: i === 0 ? color : 'rgba(10,10,10,.5)' }}>{i + 1}</td>
                                    <td style={{ padding: '10px 8px', textAlign: 'right', fontWeight: 600, color: '#0A0A0A' }}>{row.department.name}</td>
                                    <td style={{ padding: '10px 4px', textAlign: 'center', color: 'rgba(10,10,10,.6)' }}>{row.played}</td>
                                    <td style={{ padding: '10px 4px', textAlign: 'center', color: '#0A0A0A', fontWeight: 600 }}>{row.won}</td>
                                    <td style={{ padding: '10px 4px', textAlign: 'center', color: 'rgba(10,10,10,.5)' }}>{row.drawn}</td>
                                    <td style={{ padding: '10px 4px', textAlign: 'center', color: '#EF4444', fontWeight: 600 }}>{row.lost}</td>
                                    <td style={{ padding: '10px 4px', textAlign: 'center', color: 'rgba(10,10,10,.6)' }}>{row.gf}</td>
                                    <td style={{ padding: '10px 4px', textAlign: 'center', color: 'rgba(10,10,10,.6)' }}>{row.ga}</td>
                                    <td style={{ padding: '10px 4px', textAlign: 'center', fontWeight: 600, color: row.gd > 0 ? '#0A0A0A' : row.gd < 0 ? '#EF4444' : 'rgba(10,10,10,.5)' }}>
                                        {row.gd > 0 ? '+' : ''}{row.gd}
                                    </td>
            ...
            {activeView === 'matches' && (() => {
                ...
                const getRoundLabel = (roundNum: string, roundMatches: LeagueMatch[]) => {
                    if (isKnockout) {
return roundMatches[0]?.round_label ?? `الدور ${roundNum}`;
}

                    if (league.format === 'double_round_robin') {
return roundNum === '1' ? 'الذهاب' : 'الإياب';
}
                            ...
                            return (
                                <div key={roundNum} className="section">
                                    <div className="section-head">
                                        <div className="section-title" style={{ fontSize: 13, color: 'rgba(10,10,10,.5)' }}>
                                        ...
                                        <div key={match.id} className="card">
                                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
                                                ...
                                                <div style={{
                                                    ...
                                                    background: match.status === 'played' ? `${color}12` : '#F6F8F5',
                                                    color: match.status === 'played' ? color : 'rgba(10,10,10,.5)',
                                                }}>
                                                    {formatScore(match)}
                                                </div>
                                                <div style={{ flex: 1, textAlign: 'center', fontWeight: 600, fontSize: 14 }}>
                                    ...
                                    {thirdPlace && (
                                        <div>
                                            <div style={{ fontSize: 12, fontWeight: 600, color: 'rgba(10,10,10,.5)', marginBottom: 8, marginTop: 12 }}>المركز الثالث</div>
                                            <div className="card">
                                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
                                                    <div style={{ flex: 1, textAlign: 'center', fontWeight: 600, fontSize: 14 }}>
                                                    ...
                                                    <div style={{
                                                        minWidth: 70, textAlign: 'center', padding: '6px 12px',
                                                        borderRadius: 10, fontWeight: 700, fontSize: 16,
                                                        background: thirdPlace.status === 'played' ? `${color}12` : '#F6F8F5',
                                                        color: thirdPlace.status === 'played' ? color : 'rgba(10,10,10,.5)',
                                                    }}>
                        ...
                        {Object.entries(rounds).map(([roundNum, roundMatches]) => {
                            ...
                            return (
                                <div key={roundNum} style={{ display: 'flex', flexDirection: 'column', minWidth: 190 }}>
                                    <div style={{
                                        fontSize: 12, fontWeight: 600, color: '#0A0A0A', textAlign: 'center',
                                    ...
                                    {thirdPlace && (
                                        <div style={{ marginTop: 16 }}>
                                            <div style={{
                                                fontSize: 12, fontWeight: 600, color: 'rgba(10,10,10,.5)', textAlign: 'center',
                                                marginBottom: 8, background: '#F6F8F5', borderRadius: 10, padding: '5px 10px',
                                            }}>
                                                المركز الثالث
                                            </div>
                ...
                >
                    ...
                    >
                        ...
                        {editingMatch.round_label && (
                            <div style={{ fontSize: 13, color: 'rgba(10,10,10,.5)', marginBottom: 20, textAlign: 'center', background: '#F6F8F5', borderRadius: 10, padding: '8px 12px' }}>
                        ...
                        <form onSubmit={submitScore}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 20 }}>
                                <div style={{ flex: 1, textAlign: 'center' }}>
                                    <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 10, color: 'rgba(10,10,10,.6)' }}>{editingMatch.department_a?.name ?? '—'}</div>
                                    <input
                                        type="number"
                                        min="0"
                                ...
                                <div style={{ fontSize: 24, fontWeight: 700, color: '#0A0A0A', marginTop: 28 }}>:</div>
                                <div style={{ flex: 1, textAlign: 'center' }}>
                                    <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 10, color: 'rgba(10,10,10,.6)' }}>{editingMatch.department_b?.name ?? '—'}</div>
                                    <input
                                        type="number"
                                        min="0"
                            ...
                            {isKnockout && isDraw && (
                                <div style={{ marginBottom: 20 }}>
                                    <div style={{ fontSize: 13, fontWeight: 600, textAlign: 'center', color: '#C87D00', marginBottom: 12, background: '#FEF9E0', borderRadius: 10, padding: '8px 12px' }}>
                                        ...
                                        <div style={{ flex: 1, textAlign: 'center' }}>
                                            <input
                                                ...
                                                style={{
                                                    ...
                                                    borderColor: 'rgba(200,125,0,.3)',
                                                    background: '#FEF9E0',
                                                }}
                                            />
                                        </div>
                                        <div style={{ fontSize: 20, fontWeight: 700, color: '#C87D00' }}>:</div>
                                        <div style={{ flex: 1, textAlign: 'center' }}>
                                            <input
                                                type="number"
                                                min="0"
...
function BracketMatch({ match, color, isLeader, onEdit }: { match: LeagueMatch; color: string; isLeader: boolean; onEdit: (m: LeagueMatch) => void }) {
    ...
    return (
        <div style={{
            ...
            background: '#fff', border: isPlayed ? `1px solid ${color}33` : '0.5px solid rgba(10,10,10,.1)',
        }}>
            {/* Team A */}
            <div style={{
                display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                padding: '10px 14px', borderBottom: '0.5px solid rgba(10,10,10,.1)',
                background: winnerA ? `${color}08` : '#fff',
            }}>
                <span style={{
                    fontSize: 13, fontWeight: winnerA ? 700 : 400,
                    color: match.department_a_id ? (winnerA ? '#0A0A0A' : 'rgba(10,10,10,.6)') : 'rgba(10,10,10,.1)',
                }}>
                    {match.department_a?.name ?? '—'}
                </span>
                <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                    {hasPenalties && (
                        <span style={{ fontSize: 10, color: '#C87D00', fontWeight: 600 }}>({match.penalty_a})</span>
                    )}
                    <span style={{
                        fontSize: 14, fontWeight: 700, minWidth: 22, textAlign: 'center',
                        color: winnerA ? color : 'rgba(10,10,10,.5)',
                    }}>
                        {match.score_a ?? ''}
                    </span>
                </span>
            ...
            <div style={{
                ...
                <span style={{
                    ...
                    color: match.department_b_id ? (winnerB ? '#0A0A0A' : 'rgba(10,10,10,.6)') : 'rgba(10,10,10,.1)',
                }}>
                    {match.department_b?.name ?? '—'}
                </span>
                <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                    {hasPenalties && (
                        <span style={{ fontSize: 10, color: '#C87D00', fontWeight: 600 }}>({match.penalty_b})</span>
                    )}
                    <span style={{
                        fontSize: 14, fontWeight: 700, minWidth: 22, textAlign: 'center',
                        color: winnerB ? color : 'rgba(10,10,10,.5)',
                    }}>
                        {match.score_b ?? ''}
                    </span>
                </span>
            ...
            {isLeader && hasTeams && (
                <div
                    ...
                    style={{
                        ...
                        color, cursor: 'pointer', borderTop: '0.5px solid rgba(10,10,10,.1)',
                        background: '#F6F8F5',
                    }}
                >
