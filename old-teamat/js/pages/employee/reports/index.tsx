    ...
    return (
        <EmployeeLayout>
            ...
            <div style={{ marginBottom: 28 }}>
                <h1 className="text-lg font-black text-[#0A0A0A]">📊 تقاريري</h1>
                <p className="text-[11px] text-[#0A0A0A]/55 mt-0.5">{employee.name}</p>
            </div>
            ...
            <div ref={budgetRef} className="card" style={{ marginBottom: 16, padding: 0, overflow: 'hidden' }}>
                <div style={{ padding: '14px 16px', borderBottom: '0.5px solid rgba(10,10,10,.1)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ padding: 16 }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                        <span style={{ fontSize: 13, color: 'rgba(10,10,10,.5)' }}>هذا الشهر</span>
                        <span style={{ fontSize: 14, fontWeight: 600, color: '#0A0A0A' }}>{budget.this_month_used.toLocaleString()} ر</span>
                    </div>
                    ...
                    {/* Meta */}
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, color: 'rgba(10,10,10,.5)', marginBottom: 16 }}>
                    ...
                    <div style={{ height: 1, background: '#F6F8F5', marginBottom: 14 }} />

                    {/* Renewal date */}
                    <div style={{ fontSize: 13, color: 'rgba(10,10,10,.5)', marginBottom: 14 }}>
            ...
            <div ref={statsRef} className="card" style={{ marginBottom: 16, padding: 0, overflow: 'hidden' }}>
                ...
                <div style={{ padding: 16 }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                        {/* Favorite Activity */}
                        <div className="card" style={{ background: '#F6F8F5', marginBottom: 0 }}>
                            <div style={{ fontSize: 12, color: 'rgba(10,10,10,.5)', marginBottom: 8, fontWeight: 500 }}>نشاطي المفضل</div>
                            {myStats.favorite_activity ? (
                                <>
                                    ...
                                    <div style={{ fontSize: 12, color: 'rgba(10,10,10,.5)' }}>{myStats.favorite_activity.count} من {myStats.total_activities} نشاط</div>
                                </>
                            ) : (
                                <div style={{ fontSize: 13, color: 'rgba(10,10,10,.5)' }}>لا يوجد بعد</div>
                            )}
                        </div>

                        {/* Longest Streak */}
                        <div className="card" style={{ background: '#F6F8F5', marginBottom: 0 }}>
                            <div style={{ fontSize: 12, color: 'rgba(10,10,10,.5)', marginBottom: 8, fontWeight: 500 }}>أطول streak</div>
                            <div style={{ fontSize: 22, marginBottom: 4 }}>🔥</div>
                            <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 2 }}>{myStats.longest_streak} أسابيع</div>
                            <div style={{ fontSize: 12, color: 'rgba(10,10,10,.5)' }}>متواصلة</div>
                        </div>

                        {/* This Month */}
                        <div className="card" style={{ background: '#F6F8F5', marginBottom: 0 }}>
                            <div style={{ fontSize: 12, color: 'rgba(10,10,10,.5)', marginBottom: 8, fontWeight: 500 }}>هذا الشهر</div>
                            <div style={{ fontSize: 28, fontWeight: 700, color: '#0A0A0A', letterSpacing: '-1px', lineHeight: 1, marginBottom: 4 }}>{myStats.events_this_month}</div>
                            <div style={{ fontSize: 12, color: 'rgba(10,10,10,.5)' }}>أنشطة</div>
                        </div>

                        {/* Community Rank */}
                        <div className="card" style={{ background: '#F6F8F5', marginBottom: 0 }}>
                            <div style={{ fontSize: 12, color: 'rgba(10,10,10,.5)', marginBottom: 8, fontWeight: 500 }}>ترتيبي في المجتمع</div>
                            {myStats.community_rank ? (
                                <>
                                    ...
                                    <div style={{ fontSize: 12, color: 'rgba(10,10,10,.5)' }}>من {myStats.community_rank.total} عضو</div>
                                </>
                            ) : (
                                <div style={{ fontSize: 13, color: 'rgba(10,10,10,.5)' }}>غير مصنّف بعد</div>
                            )}
                        </div>
                    </div>
                </div>
            ...
            <div ref={activityRef} className="card" style={{ padding: 0, overflow: 'hidden', marginBottom: 24 }}>
                ...
                {/* Filter Buttons */}
                <div style={{ padding: '12px 16px', borderBottom: '0.5px solid rgba(10,10,10,.1)', display: 'flex', gap: 8, overflowX: 'auto' }}>
                    <button
                                ...
                                >
                                    ...
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                        <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 2 }}>
                                            {item.category_name}
                                            {item.partner_name && (
                                                <span style={{ fontWeight: 400, color: 'rgba(10,10,10,.5)' }}>
                                                    {' — '}{item.partner_name}
                                                </span>
                                            )}
                                        </div>
                                        <div style={{ fontSize: 12, color: 'rgba(10,10,10,.5)', lineHeight: 1.6 }}>
                                    ...
                                    {/* Amount */}
                                    <div style={{ fontWeight: 700, fontSize: 14, color: '#0A0A0A', flexShrink: 0 }}>
