...
import { BarChart3, Bell, ChevronLeft, Wallet } from 'lucide-react';
...
const statusMap: Record<string, { label: string; color: string }> = {
    pending_approval: { label: 'بانتظار الاعتماد', color: '#C87D00' },
    open: { label: 'مفتوح', color: '#0A0A0A' },
    pending_provider: { label: 'بانتظار المزوّد', color: '#C87D00' },
    provider_alternative: { label: 'بديل مقترح', color: '#C87D00' },
    booked: { label: 'محجوزة', color: '#0A0A0A' },
    awaiting_payment: { label: 'بانتظار الدفع', color: '#C87D00' },
    confirmed: { label: 'مؤكد', color: '#0A0A0A' },
    in_progress: { label: 'جارية الآن', color: '#0A0A0A' },
    completed: { label: 'مكتمل', color: 'rgba(10,10,10,.6)' },
    settled: { label: 'مسوّاة', color: 'rgba(10,10,10,.6)' },
        ...
        <EmployeeLayout>
            ...
            <div style={{ paddingTop: 8, paddingBottom: 24, textAlign: 'center' }}>
                <div style={{ position: 'relative', display: 'inline-block', marginBottom: 12 }}>
                    {employee.avatar ? (
                        ...
                        <div className="avatar" style={{ width: 64, height: 64, margin: '0 auto', background: '#0A0A0A', color: '#fff', fontSize: 26, fontWeight: 700 }}>
                    ...
                    {editing && (
                        <button
                            ...
                            style={{ position: 'absolute', bottom: -4, right: -4, width: 28, height: 28, borderRadius: '50%', background: '#0A0A0A', color: '#fff', border: 'none', cursor: 'pointer', fontSize: 14, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                        >
                            📷
                        </button>
                    )}
                ...
                <div style={{ fontSize: 14, color: 'rgba(10,10,10,.6)', marginTop: 4 }}>
            ...
            <div className="metrics" style={{ gridTemplateColumns: 'repeat(3, 1fr)', marginBottom: 20 }}>
                <div className="metric" style={{ textAlign: 'center' }}>
                    <div className="value" style={{ color: '#0A0A0A' }}>{stats.events_participated}</div>
                    <div className="label">فعالية شاركت</div>
                </div>
                <div className="metric" style={{ textAlign: 'center' }}>
                    <div className="value" style={{ color: '#0A0A0A' }}>{stats.communities_joined}</div>
                    <div className="label">مجتمعات</div>
                </div>
                <div className="metric" style={{ textAlign: 'center' }}>
                    <div className="value" style={{ color: '#0A0A0A' }}>{stats.events_created}</div>
            ...
            <div className="metrics" style={{ gridTemplateColumns: 'repeat(3, 1fr)', marginBottom: 20 }}>
                <div className="metric" style={{ textAlign: 'center' }}>
                    <div className="value" style={{ fontSize: 20, color: '#0A0A0A' }}>{activityStats.total_events}</div>
                    <div className="label">إجمالي الفعاليات</div>
                </div>
                <div className="metric" style={{ textAlign: 'center' }}>
                    <div className="value" style={{ fontSize: 20, color: '#0A0A0A' }}>{activityStats.events_this_month}</div>
                    <div className="label">هذا الشهر</div>
                </div>
                <div className="metric" style={{ textAlign: 'center' }}>
                    <div className="value" style={{ fontSize: 14, color: '#0A0A0A' }}>{activityStats.top_category ?? '—'}</div>
            ...
            {editing ? (
                <form onSubmit={handleSubmit} style={{ marginBottom: 20 }}>
                    ...
                    <div style={{ marginBottom: 12 }}>
                        <label style={{ fontSize: 13, color: 'rgba(10,10,10,.6)', marginBottom: 6, display: 'block' }}>الاسم</label>
                        <input
                    ...
                    <div style={{ marginBottom: 16 }}>
                        <label style={{ fontSize: 13, color: 'rgba(10,10,10,.6)', marginBottom: 6, display: 'block' }}>رقم الجوال</label>
                        <input
            ...
            {/*
                «سجل مشاركاتي والعمليات» — the prototype's account screen holds
                these; the bottom tab bar is five-up, so payments and reports
                are reached from here rather than from the bar itself.
            */}
            <div className="bg-white rounded-2xl border-[0.5px] border-[#0A0A0A]/10 overflow-hidden mb-5">
                <div className="px-4 py-3 border-b-[0.5px] border-[#0A0A0A]/10 text-xs font-extrabold text-[#0A0A0A]">
                    سجل مشاركاتي والعمليات
                </div>
                <div className="divide-y-[0.5px] divide-[#0A0A0A]/10">
                    {[
                        { href: '/employee/payments', icon: Wallet, label: 'مدفوعاتي', hint: 'الحصص المدفوعة والاستردادات' },
                        { href: '/employee/reports', icon: BarChart3, label: 'تقاريري', hint: 'مشاركاتي وحضوري عبر الفترات' },
                        { href: '/employee/notifications', icon: Bell, label: 'الإشعارات', hint: 'كل ما وصلك من المنصة' },
                    ].map((row) => (
                        <Link
                            key={row.href}
                            href={row.href}
                            className="flex items-center gap-3 px-4 py-3 hover:bg-[#0A0A0A]/[0.03] transition-colors"
                        >
                            <row.icon className="w-4 h-4 text-[#0A0A0A]/50 shrink-0" aria-hidden="true" />
                            <span className="min-w-0 flex-1">
                                <span className="block text-xs font-bold text-[#0A0A0A]">{row.label}</span>
                                <span className="block text-[11px] text-[#0A0A0A]/50">{row.hint}</span>
                            </span>
                            <ChevronLeft className="w-4 h-4 text-[#0A0A0A]/30 shrink-0" aria-hidden="true" />
                        </Link>
                    ))}
                </div>
            </div>
                        ...
                        <div className="list-card">
                            {events.map((event) => {
                                ...
                                const pct = event.capacity > 0
                                    ...
                                    : 0;
                                const statusInfo = statusMap[event.status] ?? { label: event.status, color: 'rgba(10,10,10,.6)' };
                                    ...
                                    >
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                                            ...
                                            <div style={{ textAlign: 'right' }}>
                                                ...
                                                <div style={{ fontSize: 12, color: 'rgba(10,10,10,.5)', marginTop: 2 }}>{event.community?.name}</div>
                                            </div>
                                        </div>

                                        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 14, marginBottom: 10, fontSize: 13, color: 'rgba(10,10,10,.6)' }}>
                                        ...
                                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: 'rgba(10,10,10,.5)', marginTop: 6 }}>
                    ...
                    {communities.length > 0 ? (
                        <div className="list-card">
                            {communities.map((community) => {
                                const cColor = community.category?.color ?? community.color ?? '#0A0A0A';
                        ...
                        <div className="empty">
                            ...
                            <div className="txt">
                                ...
                                <Link href="/employee/explore" style={{ color: '#0A0A0A', fontWeight: 600 }}>اكتشف المجتمعات</Link>
                            </div>
                        </div>
                    )}
                </div>
...
function NotificationPreferences({ rows }: { rows: NotificationPreferenceRow[] }) {
    ...
    return (
        <div className="list-card">
            <div style={{ padding: '14px 16px', fontSize: 13, color: 'rgba(10,10,10,.6)', lineHeight: 1.9 }}>
            ...
            {rows.map((row) => (
                ...
                >
                    ...
                    <span style={{ flex: 1 }}>
                        ...
                        {row.audience && <span style={{ fontSize: 12, color: 'rgba(10,10,10,.5)' }}>{row.audience}</span>}
                    </span>
                </label>
            ))}
