    ...
    return (
        <EmployeeLayout>
            ...
            <div style={{ marginBottom: 24 }}>
                <h1 className="text-lg font-black text-[#0A0A0A]">🏆 لوحتا الصدارة</h1>
            ...
            <div className="card" style={{ marginBottom: 16, display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'flex-end' }}>
                <div style={{ minWidth: 180 }}>
                    <label style={{ fontSize: 12, color: 'rgba(10,10,10,.6)', display: 'block', marginBottom: 4 }}>المجتمع</label>
                    <select
                        value={community?.id ?? ''}
                        onChange={(e) => pick({ community: Number(e.target.value) })}
                        style={{ width: '100%', padding: '8px 10px', border: '0.5px solid rgba(10,10,10,.15)', borderRadius: 8 }}
                    >
                        {communities.map((c) => (
                ...
                <div style={{ minWidth: 220 }}>
                    <label style={{ fontSize: 12, color: 'rgba(10,10,10,.6)', display: 'block', marginBottom: 4 }}>الموسم</label>
                    <select
                        value={season?.id ?? ''}
            ...
            {showSeasonForm && canManageSeasons && community && (
                <form className="card" onSubmit={submitSeason} style={{ marginBottom: 16, display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'flex-end' }}>
                    <div style={{ minWidth: 200 }}>
                        <label style={{ fontSize: 12, color: 'rgba(10,10,10,.6)', display: 'block', marginBottom: 4 }}>اسم الموسم</label>
                        <input
                            value={seasonForm.data.name}
                            onChange={(e) => seasonForm.setData('name', e.target.value)}
                            style={{ width: '100%', padding: '8px 10px', border: '0.5px solid rgba(10,10,10,.15)', borderRadius: 8 }}
                        />
                        {seasonForm.errors.name && <div style={{ color: '#D9381E', fontSize: 12 }}>{seasonForm.errors.name}</div>}
                    </div>
                    <div>
                        <label style={{ fontSize: 12, color: 'rgba(10,10,10,.6)', display: 'block', marginBottom: 4 }}>من</label>
                        <input type="date" value={seasonForm.data.starts_on} onChange={(e) => seasonForm.setData('starts_on', e.target.value)}
                            style={{ padding: '8px 10px', border: '0.5px solid rgba(10,10,10,.15)', borderRadius: 8 }} />
                    </div>
                    <div>
                        <label style={{ fontSize: 12, color: 'rgba(10,10,10,.6)', display: 'block', marginBottom: 4 }}>إلى</label>
                        <input type="date" value={seasonForm.data.ends_on} onChange={(e) => seasonForm.setData('ends_on', e.target.value)}
                            style={{ padding: '8px 10px', border: '0.5px solid rgba(10,10,10,.15)', borderRadius: 8 }} />
                        {seasonForm.errors.ends_on && <div style={{ color: '#D9381E', fontSize: 12 }}>{seasonForm.errors.ends_on}</div>}
                    </div>
            ...
            {boards?.archived && (
                <div className="card" style={{ marginBottom: 16, background: '#FEF9E0', border: '1px solid #FEF9E0', fontSize: 13 }}>
            ...
            <div className="card" style={{ padding: 0, overflowX: 'auto' }}>
                {board === 'consistency' && (
                    consistencyRows.length === 0 ? (
                        <div style={{ padding: 24, textAlign: 'center', color: 'rgba(10,10,10,.55)', fontSize: 14 }}>
                            لا مشاركات محتسبة في هذا الموسم بعد — نقاطك تبدأ من أول مشاركة.
                        </div>
                    ) : (
                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
                            <thead>
                                <tr style={{ background: '#F6F8F5', textAlign: 'right' }}>
                            ...
                            <tbody>
                                {consistencyRows.map((row, i) => (
                                    <tr
                                        ...
                                        style={{
                                            borderTop: '1px solid #F6F8F5',
                                            background: row.employee_id === myEmployeeId ? '#F6F8F5' : undefined,
                                        }}
                                    >
                                        ...
                                        <td style={{ padding: '10px 14px', fontWeight: 600 }}>
                                            ...
                                            {level === 'department' && row.members_count !== undefined && (
                                                <span style={{ color: 'rgba(10,10,10,.55)', fontWeight: 400, fontSize: 12 }}> · {row.members_count} مشارك</span>
                                            )}
                                        </td>
