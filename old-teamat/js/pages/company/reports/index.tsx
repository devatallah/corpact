...
function sectionHeader(title: string, sub?: string) {
    return (
        <div style={{ padding: '14px 20px', borderBottom: '0.5px solid rgba(10,10,10,.1)' }}>
            ...
            {sub && <div style={{ fontSize: 11, color: 'rgba(10,10,10,.55)', marginTop: 3 }}>{sub}</div>}
        </div>
    );
}
    ...
    return (
        <CompanyLayout>
            ...
            <div style={{ marginBottom: 20 }}>
                <PageHeader title={<>التقارير والمؤشرات</>} subtitle={<>{company.name} — {period.label} (بتوقيت الرياض)</>} />
            </div>

            <div className="card" style={{ marginBottom: 16, display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'flex-end' }}>
                <div>
                    <label style={{ fontSize: 12, color: 'rgba(10,10,10,.6)', display: 'block', marginBottom: 4 }}>الفترة</label>
                    <select
                        value={period.key}
                        onChange={(e) => changePeriod(e.target.value)}
                        style={{ padding: '8px 10px', border: '0.5px solid rgba(10,10,10,.15)', borderRadius: 8, minWidth: 160 }}
                    >
                ...
                <span style={{ flex: 1 }} />
                <div>
                    <label style={{ fontSize: 12, color: 'rgba(10,10,10,.6)', display: 'block', marginBottom: 6 }}>التصدير</label>
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                        {exports.map((item) => (
                            <span key={item.key} style={{ display: 'inline-flex', border: '0.5px solid rgba(10,10,10,.15)', borderRadius: 8, overflow: 'hidden' }}>
                                <span style={{ padding: '8px 10px', fontSize: 12, background: '#F6F8F5' }}>{item.title}</span>
                                <a
                                    href={`/company/reports/export/${item.key}?format=xlsx&period=${period.key}`}
                                    style={{ padding: '8px 10px', fontSize: 12, borderRight: '0.5px solid rgba(10,10,10,.15)', color: '#0A0A0A' }}
                                >
                                    Excel
                                </a>
                                <a
            ...
            <div className="card" style={{ background: '#FEF9E0', border: '1px solid #FEF9E0', fontSize: 13, marginBottom: 16 }}>
                ...
                <table className="portal-table">
                    ...
                    <tbody>
                        {kpi.department_participation.length === 0 ? (
                            <tr>
                                <td colSpan={4} style={{ ...tdStyle, textAlign: 'center', color: 'rgba(10,10,10,.55)', padding: 24 }}>
                                    ...
                                    <td style={tdStyle}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                            <div className="bar-wrap" style={{ flex: 1, maxWidth: 160 }}>
                                                <div className="bar-fill" style={{ width: `${Math.min(100, row.rate)}%`, background: row.rate === 0 ? '#D9381E' : '#2E7D32' }} />
                                            </div>
                            ...
                            {kpi.communities.active.length === 0 && kpi.communities.dormant.length === 0 ? (
                                ...
                                [
                                    ...
                                    <tr key={row.id}>
                                        ...
                                        <td style={tdStyle}>
                                            <span className="pill" style={{ background: row.active ? '#E8F5E9' : '#FDEDEC', color: row.active ? '#2E7D32' : '#D9381E' }}>
                                                {row.active ? 'نشط' : 'خامل'}
                                            </span>
                                            {row.leaderless_dormant && (
                                                <span style={{ fontSize: 11, color: '#C87D00', marginRight: 6 }}>بلا قائد</span>
                                            )}
                                        </td>
                    ...
                    <tbody>
                        ...
                        {monthlyReports.data.map((report) => (
                            <tr key={report.id}>
                                ...
                                <td style={tdStyle}>
                                    <Link href={`/company/reports/monthly/${report.id}`} style={{ color: '#0A0A0A', fontSize: 12 }}>
                                        عرض التقرير
                                    </Link>
                                </td>
                            </tr>
