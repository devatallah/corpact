    ...
    return (
        <CompanyLayout>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                <div>
                    <PageHeader title={<>إدارة المجتمعات</>} subtitle={<>{communities.total} مجتمعات نشطة</>} />
                </div>
                <button
                    onClick={() => { setShowCreate(true); setEditingItem(null); form.reset(); setLeaderQuery(''); setSelectedLeaderName(''); setLeaderResults([]); }}
                    style={{ background: '#0A0A0A', color: '#fff', border: 'none', borderRadius: 12, padding: '10px 20px', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}
                >
                    + إنشاء مجتمع
                </button>
            </div>
            ...
            {communities.data.length === 0 ? (
                <div className="card" style={{ textAlign: 'center', padding: 32 }}>
                    <div style={{ fontSize: 13, color: 'rgba(10,10,10,.55)' }}>لا توجد مجتمعات مطابقة</div>
                </div>
            ) : (
                                ...
                                <div style={{ padding: '28px 28px 24px' }}>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 8 }}>
                                        ...
                                        <button
                                            ...
                                            style={{ background: '#F6F8F5', border: 'none', borderRadius: 10, padding: '8px 16px', fontSize: 13, fontWeight: 600, color: '#0A0A0A', cursor: 'pointer', fontFamily: 'inherit' }}
                                        >
                                            🔄 القوالب
                                        </button>
                                        <button
                                            onClick={(e) => { e.stopPropagation(); setEditingItem(community); setShowCreate(false); }}
                                            style={{ background: '#F6F8F5', border: 'none', borderRadius: 10, padding: '8px 16px', fontSize: 13, fontWeight: 600, color: 'rgba(10,10,10,.55)', cursor: 'pointer', fontFamily: 'inherit' }}
                                        >
                                            تعديل
                                        </button>
                                    </div>
                                    ...
                                    {/* Leader */}
                                    <div style={{ fontSize: 14, color: 'rgba(10,10,10,.5)', marginBottom: 24, paddingRight: 54 }}>
                                        {community.leader?.name ?? '\u2014'}
                                    </div>

                                    {/* Stats */}
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 0, background: '#F6F8F5', borderRadius: 14, padding: '20px 0' }}>
                                        <div style={{ textAlign: 'center', borderLeft: '1px solid #F6F8F5' }}>
                                            <div style={{ fontSize: 26, fontWeight: 800, color: '#0A0A0A' }}>{activeMembers}</div>
                                            <div style={{ fontSize: 12, color: 'rgba(10,10,10,.5)', marginTop: 4, fontWeight: 500 }}>عضو</div>
                                        </div>
                                        <div style={{ textAlign: 'center', borderLeft: '1px solid #F6F8F5' }}>
                                            <div style={{ fontSize: 26, fontWeight: 800, color: '#0A0A0A' }}>{eventCount}</div>
                                            <div style={{ fontSize: 12, color: 'rgba(10,10,10,.5)', marginTop: 4, fontWeight: 500 }}>فعالية</div>
                                        </div>
                                        <div style={{ textAlign: 'center' }}>
                                            <div style={{ fontSize: 22, fontWeight: 800, color }}>{balance.toLocaleString()}</div>
                                            <div style={{ fontSize: 12, color: 'rgba(10,10,10,.5)', marginTop: 4, fontWeight: 500 }}>ريال</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div className="fg" ref={leaderRef} style={{ position: 'relative' }}>
                                    ...
                                    {showLeaderDropdown && leaderResults.length > 0 && (
                                        <div style={{
                                            ...
                                            background: '#fff', border: '0.5px solid rgba(10,10,10,.1)', borderRadius: 10,
                                            ...
                                            {leaderResults.map((emp) => (
                                                <div
                                                    ...
                                                    style={{
                                                        ...
                                                        background: String(emp.id) === form.data.leader_id ? '#0A0A0A08' : undefined,
                                                    }}
                                                    onMouseEnter={(e) => (e.currentTarget.style.background = '#F6F8F5')}
                                                    onMouseLeave={(e) => (e.currentTarget.style.background = String(emp.id) === form.data.leader_id ? '#0A0A0A08' : '')}
                                                >
                                                    <div style={{ fontWeight: 600 }}>{emp.name}</div>
                                                    <div style={{ fontSize: 11, color: 'rgba(10,10,10,.55)' }}>{emp.email}</div>
                                                </div>
                                            ))}
                                        </div>
                                    )}
