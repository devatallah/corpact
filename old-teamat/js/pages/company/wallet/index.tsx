...
const inputStyle: React.CSSProperties = {
    width: '100%', padding: '10px 14px', borderRadius: 10, border: '0.5px solid rgba(10,10,10,.1)',
    fontSize: 14, background: '#FFFFFF', outline: 'none', direction: 'rtl',
};
...
export default function WalletIndex({ wallet, communities, transactions, topupRequests }: Props) {
    ...
    const COLORS = ['#2E7D32', '#C87D00', '#0A0A0A', '#0A0A0A', '#D9381E', '#0A0A0A'];
    ...
    return (
        <CompanyLayout>
            ...
            <PageHeader title={<>المحفظة والدعم</>} subtitle={<>إدارة الميزانية وتخصيص رصيد المجتمعات — كل حركة تُسجَّل في الدفتر</>} />

            {/* Balance Card */}
            <div style={{ background: 'linear-gradient(135deg,#0A0A0A,#0A0A0A)', borderRadius: 20, padding: '24px 28px', marginBottom: 20, color: '#0A0A0A' }}>
                ...
                <div style={{ marginTop: 16, textAlign: 'left' }}>
                    <button
                        ...
                        style={{ background: 'rgba(255,255,255,.2)', color: '#0A0A0A', border: '1px solid rgba(255,255,255,.3)', borderRadius: 10, padding: '8px 18px', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}
                    >
                        + طلب شحن (تحويل بنكي)
                    </button>
                </div>
            ...
            {showTopup && (
                <div style={{ background: '#fff', border: '1px solid #0A0A0A44', borderRadius: 16, padding: 20, marginBottom: 16 }}>
                    <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 4 }}>طلب شحن بتحويل بنكي</div>
                    <div style={{ fontSize: 12, color: 'rgba(10,10,10,.55)', marginBottom: 14 }}>
            ...
            {topupRequests.length > 0 && (
                <div style={{ background: '#fff', border: '0.5px solid rgba(10,10,10,.1)', borderRadius: 16, padding: 22, marginBottom: 16 }}>
                    <div style={{ fontSize: 15, fontWeight: 800, marginBottom: 14 }}>طلبات الشحن</div>
                    {topupRequests.map((request, index) => (
                        <div key={request.id} style={{ padding: '10px 0', ...(index < topupRequests.length - 1 ? { borderBottom: '0.5px solid rgba(10,10,10,.1)' } : {}) }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <div>
                                    <div style={{ fontSize: 13, fontWeight: 700 }}>
                                        {request.amount.toLocaleString()} ريال
                                        <span style={{ fontSize: 11, color: 'rgba(10,10,10,.55)', fontWeight: 500 }}> — مرجع {request.bank_reference} · حساب ****{request.sender_account_last4}</span>
                                    </div>
                                    <div style={{ fontSize: 11, color: 'rgba(10,10,10,.55)' }}>تاريخ التحويل: {request.transfer_date ?? '—'}</div>
                                </div>
                                <span style={{
                                    fontSize: 11, fontWeight: 800, padding: '4px 12px', borderRadius: 999,
                                    color: STATUS_COLORS[request.status] ?? 'rgba(10,10,10,.55)',
                                    background: `${STATUS_COLORS[request.status] ?? 'rgba(10,10,10,.55)'}18`,
                                }}>
                                    {request.status_label}
                                </span>
                            </div>
                            {request.status === 'rejected' && request.rejection_reason && (
                                <div style={{ marginTop: 6, fontSize: 12, color: '#D9381E' }}>سبب الرفض: {request.rejection_reason}</div>
                            )}
                        </div>
                    ))}
                </div>
            ...
            <div style={{ background: '#fff', border: '2px solid #0A0A0A33', borderRadius: 16, padding: 22, marginBottom: 16 }}>
                <div style={{ fontSize: 15, fontWeight: 800, marginBottom: 4 }}>تخصيص رصيد لمجتمع</div>
                <div style={{ fontSize: 12, color: 'rgba(10,10,10,.55)', marginBottom: 16 }}>اختر المجتمع وحدد المبلغ — قيد تخصيص من المحفظة الرئيسية إلى محفظة المجتمع الفرعية</div>
                <form onSubmit={handleDistribute}>
                    {communities.length === 0 ? (
                        <div style={{ fontSize: 13, color: 'rgba(10,10,10,.55)' }}>لا توجد مجتمعات</div>
                    ) : (
                                ...
                                return (
                                    <div
                                        ...
                                        style={{
                                            ...
                                            border: `2px solid ${isSelected ? color : '#F6F8F5'}`,
                                            background: isSelected ? `${color}12` : '#F6F8F5',
                                        }}
                                    >
                                        <CategoryIcon icon={community.category?.icon} size={20} />
                                        <span style={{ fontSize: 13, fontWeight: 700, color: isSelected ? color : 'rgba(10,10,10,.6)' }}>
                                            {community.name}
                                        </span>
                    ...
                    <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                        <input
                            ...
                            style={{ flex: 1, padding: '10px 14px', borderRadius: 10, border: '0.5px solid rgba(10,10,10,.1)', fontSize: 15, fontWeight: 700, background: '#FFFFFF', outline: 'none', direction: 'rtl' }}
                        />
                        <button type="submit" className="inline-flex items-center justify-center gap-1.5 h-8 px-3.5 rounded-full text-xs font-bold border-[0.5px] transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#C8FF00] bg-[#0A0A0A] text-[#C8FF00] border-[#0A0A0A] hover:bg-[#0A0A0A]/90" disabled={distForm.processing}>
                            تخصيص ←
                        </button>
                    </div>
            ...
            {transactions.length > 0 && (
                <div style={{ background: '#fff', border: '0.5px solid rgba(10,10,10,.1)', borderRadius: 16, padding: 22 }}>
                    ...
                    <div style={{ fontSize: 11, color: 'rgba(10,10,10,.55)', marginBottom: 12 }}>الرصيد = مجموع الحركات — لا تعديل ولا حذف؛ التصحيح بحركة عكسية مرتبطة</div>
                    {transactions.map((tx, index) => (
                        <div
                            ...
                            style={{
                                ...
                                padding: '10px 0',
                                ...(index < transactions.length - 1 ? { borderBottom: '0.5px solid rgba(10,10,10,.1)' } : {}),
                            }}
                        >
                            <div>
                                ...
                                <div style={{ fontSize: 11, color: 'rgba(10,10,10,.55)' }}>{tx.occurred_at ? fmtDateTime(tx.occurred_at) : ''}</div>
                            </div>
                            <div style={{ fontSize: 14, fontWeight: 800, color: tx.direction === 'credit' ? '#2E7D32' : '#D9381E' }}>
