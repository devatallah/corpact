...
const rankColors = ['#C87D00', 'rgba(10,10,10,.55)', '#C87D00'];
...
export default function HrDashboard({ company, stats, communityActivity, departmentParticipation, recentActivity, leaderboard }: Props) {
    return (
        <CompanyLayout>
            ...
            <PageHeader title={<>لوحة التحكم</>} subtitle={<>{company.name} — {stats.period.label}</>} />
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 16 }}>
                <div className="card" style={{ marginBottom: 0 }}>
                    ...
                    <div style={{ fontSize: 11, color: 'rgba(10,10,10,.55)', marginBottom: 10 }}>
                        منسوبة للإدارة وقت الفعالية لا للإدارة الحالية
                    </div>
                    {departmentParticipation.length === 0 ? (
                        <div style={{ fontSize: 13, color: 'rgba(10,10,10,.55)', padding: '16px 0' }}>
                        ...
                        departmentParticipation.map((row) => (
                            <div key={row.department_id ?? 'none'} style={{ marginBottom: 12 }}>
                                ...
                                <div className="bar-w">
                                    <div className="bar-f" style={{ width: `${Math.min(100, row.rate)}%`, background: row.rate === 0 ? '#D9381E' : '#2E7D32' }} />
                                </div>
                            </div>
                        ))
                    )}
                    {communityActivity.dormant.length > 0 && (
                        <div style={{ marginTop: 16, paddingTop: 12, borderTop: '0.5px solid rgba(10,10,10,.1)' }}>
                        ...
                        recentActivity.map((activity) => (
                            <div key={activity.id} style={{ display: 'flex', gap: 10, marginBottom: 12 }}>
                                <div style={{
                                    ...
                                    background: '#2E7D3218', display: 'flex',
                                ...
                                <div>
                                    ...
                                    <div style={{ fontSize: 10, color: 'rgba(10,10,10,.55)', marginTop: 3 }}>
            ...
            {(leaderboard.top_departments.length > 0 || leaderboard.top_employees.length > 0) && (
                <div style={{ marginTop: 24 }}>
                    <h2 className="text-base font-extrabold text-[#0A0A0A] mb-4">ترتيب النشاط هذا الشهر</h2>
                        ...
                        {leaderboard.top_departments.length > 0 && (
                            <div className="card">
                                ...
                                {leaderboard.top_departments.slice(0, 5).map((dept, idx) => (
                                    <div key={dept.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: idx < leaderboard.top_departments.length - 1 ? '0.5px solid rgba(10,10,10,.1)' : 'none' }}>
                                        <div style={{ width: 24, height: 24, borderRadius: '50%', background: rankColors[idx] ?? '#F6F8F5', color: '#0A0A0A', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 800, flexShrink: 0 }}>{idx + 1}</div>
                                        <div style={{ flex: 1, fontSize: 13, fontWeight: 600 }}>{dept.name}</div>
                                        <div style={{ fontSize: 12, fontWeight: 700, color: 'rgba(10,10,10,.55)' }}>{dept.events_count}</div>
                                    </div>
                                ))}
                            </div>
                        )}
                        ...
                        {leaderboard.top_employees.length > 0 && (
                            <div className="card">
                                ...
                                {leaderboard.top_employees.slice(0, 5).map((emp, idx) => (
                                    <div key={emp.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: idx < leaderboard.top_employees.length - 1 ? '0.5px solid rgba(10,10,10,.1)' : 'none' }}>
                                        <div style={{ width: 24, height: 24, borderRadius: '50%', background: rankColors[idx] ?? '#F6F8F5', color: '#0A0A0A', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 800, flexShrink: 0 }}>{idx + 1}</div>
                                        <div style={{ flex: 1, minWidth: 0 }}>
                                            <div style={{ fontSize: 13, fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{emp.name}</div>
                                            {emp.department_name && <div style={{ fontSize: 10, color: 'rgba(10,10,10,.55)' }}>{emp.department_name}</div>}
                                        </div>
                                        <div style={{ fontSize: 12, fontWeight: 700, color: 'rgba(10,10,10,.55)' }}>{emp.events_count}</div>
                                    </div>
                                ))}
                            </div>
                        )}
