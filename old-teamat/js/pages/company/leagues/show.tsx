    ...
    return (
        <CompanyLayout>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 24 }}>
                <Link href="/company/leagues" style={{ color: 'rgba(10,10,10,.55)', textDecoration: 'none', fontSize: 13 }}>← البطولات</Link>
                <span style={{ color: '#0A0A0A' }}>/</span>
                <span style={{ fontWeight: 700 }}>{league.name}</span>
            </div>

            <div style={{ background: '#fff', border: '0.5px solid rgba(10,10,10,.1)', borderRadius: 16, padding: 20, marginBottom: 16 }}>
                ...
                {views.map((v) => (
                    <button
                        ...
                        style={{
                            ...
                            border: activeView === v.key ? `2px solid ${accent}` : '0.5px solid rgba(10,10,10,.1)',
                            background: activeView === v.key ? `${accent}10` : '#fff',
                            color: activeView === v.key ? accent : 'rgba(10,10,10,.55)',
                        }}
                    >
                        {v.label}
                    </button>
            ...
            {activeView === 'standings' && standings && (
                <div style={{ background: '#fff', border: '0.5px solid rgba(10,10,10,.1)', borderRadius: 16, overflow: 'auto' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                        <thead>
                            <tr style={{ background: '#F6F8F5', textAlign: 'center' }}>
                        ...
                        <tbody>
                            {standings.map((row, i) => (
                                <tr key={row.department.id} style={{ borderTop: '0.5px solid rgba(10,10,10,.1)', textAlign: 'center' }}>
                                    <td style={{ padding: '10px 12px', textAlign: 'right', fontWeight: 700, color: i < 1 ? accent : 'rgba(10,10,10,.6)' }}>{i + 1}</td>
                                    ...
                                    <td style={{ padding: '10px 6px', color: row.gd > 0 ? '#2E7D32' : row.gd < 0 ? '#D9381E' : 'rgba(10,10,10,.55)' }}>
                                        {row.gd > 0 ? '+' : ''}{row.gd}
                                    </td>
            ...
            {activeView === 'matches' && (() => {
                ...
                if (assignedMatches.length === 0) {
                    return <div style={{ textAlign: 'center', padding: 24, color: 'rgba(10,10,10,.55)', fontSize: 13 }}>لا توجد مباريات بعد</div>;
                }
                        ...
                        {Object.entries(grouped).map(([roundNum, roundMatches]) => {
                            ...
                            return (
                                <div key={roundNum} style={{ marginBottom: 16 }}>
                                    <div style={{ fontSize: 12, fontWeight: 700, color: 'rgba(10,10,10,.55)', marginBottom: 8, paddingRight: 4 }}>
                                        {getRoundLabel(roundNum, roundMatches)}
                                    </div>
                                    {regular.map((match) => (
                                        <div key={match.id} style={{ background: '#fff', border: '0.5px solid rgba(10,10,10,.1)', borderRadius: 12, padding: 14, marginBottom: 8 }}>
                                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
                                                ...
                                                <div style={{
                                                    ...
                                                    background: match.status === 'played' ? `${accent}10` : '#F6F8F5',
                                                    color: match.status === 'played' ? accent : 'rgba(10,10,10,.55)',
                                                }}>
                                    ...
                                    {thirdPlace && (
                                        <div>
                                            <div style={{ fontSize: 11, fontWeight: 600, color: 'rgba(10,10,10,.55)', marginBottom: 6, marginTop: 10, paddingRight: 4 }}>المركز الثالث</div>
                                            <div style={{ background: '#fff', border: '0.5px solid rgba(10,10,10,.1)', borderRadius: 12, padding: 14, marginBottom: 8 }}>
                                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
                                                    <div style={{ flex: 1, textAlign: 'center', fontWeight: 600, fontSize: 13 }}>{thirdPlace.department_a?.name ?? '—'}</div>
                                                    <div style={{
                                                        minWidth: 70, textAlign: 'center', padding: '6px 12px', borderRadius: 8, fontWeight: 900, fontSize: 16,
                                                        background: thirdPlace.status === 'played' ? `${accent}10` : '#F6F8F5',
                                                        color: thirdPlace.status === 'played' ? accent : 'rgba(10,10,10,.55)',
                                                    }}>
                        ...
                        {Object.entries(rounds).map(([roundNum, roundMatches]) => {
                            ...
                            return (
                                <div key={roundNum} style={{ display: 'flex', flexDirection: 'column', gap: 16, minWidth: 180 }}>
                                    <div style={{ fontSize: 11, fontWeight: 700, color: 'rgba(10,10,10,.55)', textAlign: 'center', marginBottom: 4 }}>{label}</div>
                                    {regular.map((match) => (
                                        <BracketMatch key={match.id} match={match} accent={accent} />
                                    ))}
                                    {thirdPlace && (
                                        <>
                                            <div style={{ fontSize: 10, color: 'rgba(10,10,10,.55)', textAlign: 'center', fontWeight: 600 }}>المركز الثالث</div>
...
function BracketMatch({ match, accent }: { match: LeagueMatch; accent: string }) {
    ...
    return (
        <div style={{ border: '0.5px solid rgba(10,10,10,.1)', borderRadius: 10, overflow: 'hidden', background: '#fff', minWidth: 170 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 10px', borderBottom: '0.5px solid rgba(10,10,10,.1)', background: winnerA ? `${accent}08` : undefined, fontWeight: winnerA ? 800 : 500 }}>
                <span style={{ fontSize: 12, color: match.department_a_id ? '#0A0A0A' : '#0A0A0A' }}>{match.department_a?.name ?? '—'}</span>
                <span style={{ fontSize: 12, fontWeight: 700, color: winnerA ? accent : 'rgba(10,10,10,.55)' }}>{match.score_a ?? ''}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 10px', background: winnerB ? `${accent}08` : undefined, fontWeight: winnerB ? 800 : 500 }}>
                <span style={{ fontSize: 12, color: match.department_b_id ? '#0A0A0A' : '#0A0A0A' }}>{match.department_b?.name ?? '—'}</span>
                <span style={{ fontSize: 12, fontWeight: 700, color: winnerB ? accent : 'rgba(10,10,10,.55)' }}>{match.score_b ?? ''}</span>
            </div>
        </div>
    );
}
