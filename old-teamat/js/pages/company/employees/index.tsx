    ...
    return (
        <CompanyLayout>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24, flexWrap: 'wrap', gap: 10 }}>
                <div>
                    <PageHeader title={<>الموظفون</>} subtitle={<>{activeCount} نشط من {totalCount}</>} />
                </div>
                <div style={{ display: 'flex', gap: 10 }}>
                    <input
                        ...
                        placeholder="ابحث..."
                        style={{ padding: '9px 14px', borderRadius: 10, border: '0.5px solid rgba(10,10,10,.1)', fontSize: 13, background: '#fff', outline: 'none', direction: 'rtl', width: 180, fontFamily: 'inherit' }}
                    />
                    <Link
                        href="/company/employees/import"
                        style={{ background: '#F6F8F5', color: '#0A0A0A', border: '0.5px solid rgba(10,10,10,.1)', borderRadius: 10, padding: '9px 18px', fontSize: 13, fontWeight: 700, textDecoration: 'none' }}
                    >
                        ⬆ استيراد ملف
                    </Link>
                    <button
                        onClick={() => setShowInvite(!showInvite)}
                        style={{ background: '#0A0A0A', color: '#fff', border: 'none', borderRadius: 10, padding: '9px 18px', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}
                    >
                        + دعوة موظف
                    </button>
                </div>
            ...
            {showInvite && (
                <div className="detail-overlay open" onClick={() => setShowInvite(false)}>
                    <div className="detail-panel" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div style={{ fontSize: 13, color: 'rgba(10,10,10,.55)', marginBottom: 16 }}>سيصله إيميل دعوة للانضمام للمنصة</div>
                            ...
                            <tr key={employee.id}>
                                <td>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                        <div style={{ width: 36, height: 36, borderRadius: '50%', background: '#0A0A0A18', color: '#0A0A0A', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 700 }}>
                                            {employee.name.charAt(0)}
                                        </div>
                                        <div>
                                            <div style={{ fontSize: 13, fontWeight: 600 }}>{employee.name}</div>
                                            <div style={{ fontSize: 11, color: 'rgba(10,10,10,.55)' }} dir="ltr">{employee.email}</div>
                                        </div>
                                    </div>
                                </td>
                                <td style={{ color: 'rgba(10,10,10,.55)', fontSize: 12 }}>
                                    ...
                                    {employee.communities && employee.communities.length > 0 ? (
                                        employee.communities.map((c) => (
                                            <span
                                                key={c.id}
                                                className="badge"
                                                style={{ background: '#2E7D3218', color: '#2E7D32', marginLeft: 4 }}
                                            >
                                                <CategoryIcon icon={c.category?.icon} size={14} /> {c.name}
                                            </span>
                                        ))
                                    ) : (
                                        <span style={{ color: 'rgba(10,10,10,.55)', fontSize: 12 }}>{'\u2014'}</span>
                                    )}
                                </td>
                                <td style={{ fontWeight: 700, ...(employee.events_count === 0 ? { color: 'rgba(10,10,10,.55)' } : {}) }}>
                                    {employee.events_count ?? 0}
                                </td>
                                <td style={{ color: 'rgba(10,10,10,.55)', fontSize: 12, whiteSpace: 'nowrap' }}>
                                ...
                                <td>
                                    <button
                                        ...
                                        style={{ background: '#0A0A0A18', color: '#0A0A0A', border: '1px solid #0A0A0A33', borderRadius: 8, padding: '5px 14px', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}
                                    >
                                        تعديل
                                    </button>
                                </td>
